 ...
 *
 * Today this is just `createPage` (Oblien Pages on shared `.vibrail.app`).
...
export class CloudRuntime implements MultiServiceRuntimeAdapter {
  ...
  // admin-scoped operations (e.g. creating pages on shared `.vibrail.app`)
...
if [ "$(ls -A /app/.staging)" ]; then
  ...
  async deployStatic(
    ...
    // live on the shared `.vibrail.app` zone (an account-level op): a namespace
  ...
  /**
   * Check whether a subdomain slug is available on vibrail.app.
