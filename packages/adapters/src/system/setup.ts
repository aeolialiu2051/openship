...
function resolveRequired(mode: RuntimeMode): string[] {
  return mode === "docker" ? ["docker", "git"] : ["git"];
}
...
export interface SystemManagerOptions {
   ...
   * become no-ops for containerized control-plane environments.
   */
...
export class SystemManager {
  ...
  async checkFeature(feature: Feature): Promise<FeatureReadiness> {
    // Docker-edge / assume-installed mode: the binaries (traefik, certbot) live
  ...
  async ensureFeature(
    ...
    // socket), not installable from inside the api — skip.
