...
async function triggerBranchDeployments(
  ...
  if (autoDeployProjects.length === 0) {
    ...
    if (!env.CLOUD_MODE) {
      ...
      if (fwd.forwarded) {
        console.log(
          `[GitHub Webhook] ${input.event} for ${input.owner}/${input.repo}#${input.branch} - forwarded to Vibrail Cloud (${fwd.cloudProjectId})`,
        );
        ...
        return {
          success: true,
          event: input.event,
          message: `Forwarded push to Vibrail Cloud (${fwd.cloudProjectId})`,
        };
      }
    }
    console.log(
