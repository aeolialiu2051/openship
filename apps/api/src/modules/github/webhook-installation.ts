...
export async function handleInstallation(
  ...
  // SaaS-only contract: the GitHub App is owned by vibrail.com, so
  ...
  // is configured to point at vibrail.com — an installation event
  ...
  if (!env.CLOUD_MODE && env.GITHUB_AUTH_MODE !== "app") {
    console.log(
      `[GitHub Webhook] Ignoring installation.${payload.action} on self-hosted instance — SaaS (vibrail.com) is the authoritative source for GitHub App installations.`,
    );
    return {
      success: true,
      event: "installation",
