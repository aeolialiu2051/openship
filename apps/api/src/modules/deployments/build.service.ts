...
export interface DeploymentConfigSnapshot {
   ...
   * deploy — e.g. a self-hosted + free-.vibrail.app deploy whose cloud edge route
...
export async function resolveSnapshotTarget(
  ...
  else if (override?.serverId) deployTarget = "server";
...
export async function requestBuildAccess(ctx: RequestContext, input: BuildAccessInput) {
  ...
  // must deploy with no public route — defaulting a free .vibrail.app project domain
