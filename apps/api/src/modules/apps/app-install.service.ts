...
export async function installApp(
  ...
  const routeKey = input.routeKey ?? generateProjectRouteKey();
  ...
  if (existingDraft) {
    if (!existingDraft.routeKey) {
      await repos.project.update(existingDraft.id, { routeKey });
    }
    return {
      kind: "template",
      projectId: existingDraft.id,
      slug: existingDraft.slug,
      routeKey: existingDraft.routeKey ?? routeKey,
    };
  }
    ...
    const publicEndpoints =
      svc.routes && svc.routes.length > 0
        ? svc.routes.map((route) => {
            const label = appServiceManagedLabel({
              projectLabel: project.slug ?? project.name,
              serviceName: svc.name,
              routeKey: project.routeKey,
