 ...
 *   - free  domain → Oblien edge proxy (slug.vibrail.app → this box), reusing
...
export async function cloudConnect(c: Context) {
  ...
  try {
    ...
    // for SERVICES ONLY — the free .vibrail.app domain and managed mail. Store the cloud
...
export async function selfRegister(c: Context) {
  ...
  if (domainType === "free") {
    ...
    try {
      ...
      if (!result) {
        return c.json(
          { error: "Vibrail Cloud is not connected — connect it to use a free .vibrail.app domain." },
          409,
        );
      }
    } catch (err) {
    ...
    // Oblien's edge terminates TLS for *.vibrail.app; the origin remains plain HTTP.
