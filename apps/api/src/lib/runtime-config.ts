...
export const runtimeConfigSchema = z.object({
  ...
  STRIPE_PRICE_PRO_MONTHLY: z.number().positive().max(100_000),
  STRIPE_PRICE_PRO_ANNUAL: z.number().positive().max(1_000_000),
  STRIPE_PRICE_PRO_PROMOTIONAL: z.number().min(0).max(100_000),
  STRIPE_PRICE_PRO_ANNUAL_PROMOTIONAL: z.number().min(0).max(1_000_000),
});
...
const environmentDefaults: EditableRuntimeConfig = {
  ...
  STRIPE_PRICE_PRO_PROMOTIONAL: env.STRIPE_PRICE_PRO_PROMOTIONAL,
  STRIPE_PRICE_PRO_ANNUAL_PROMOTIONAL: env.STRIPE_PRICE_PRO_ANNUAL_PROMOTIONAL,
};
