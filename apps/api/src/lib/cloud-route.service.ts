 ...
 * primitive, so removing an old *managed* (`*.vibrail.warpgateapi.com`) subdomain on a *dynamic*
...
export interface CloudRouteInput {
  /** Full hostname — `slug.vibrail.warpgateapi.com` (managed) or `app.example.com` (custom). */
  hostname: string;
...
export async function reapplyCloudProjectRoute(
  ...
  try {
    if (containerId.startsWith(PAGE_CONTAINER_PREFIX)) {
      // Static page: the free *.vibrail.warpgateapi.com subdomain IS the page slug (set at
    ...
    if (input.isCustomDomain) {
      ...
      // leaves only the last port's ingress open. Managed (*.vibrail.warpgateapi.com) routes
