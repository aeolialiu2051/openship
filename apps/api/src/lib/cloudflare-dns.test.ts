...
describe("waitForDeploymentDnsPropagation", () => {
  ...
  it("uses the public DNS resolver instead of trusting the API host resolver", async () => {
    ...
    await expect(
      waitForDeploymentDnsPropagation("CLIProxyAPI-CLI-Proxy-API-CVQQ59.vibrail.app.", {
        attempts: 1,
        intervalMs: 0,
      }),
    ).resolves.toBe(true);
    ...
    expect(mocks.resolveRecords).toHaveBeenCalledWith(
      "cliproxyapi-cli-proxy-api-cvqq59.vibrail.app",
      "A",
      { timeoutMs: 2_000 },
    );
  });
  ...
  it("keeps the bounded retry behavior when public DNS has not answered yet", async () => {
    ...
    await expect(
      waitForDeploymentDnsPropagation("app.vibrail.app", {
        attempts: 2,
        intervalMs: 1,
        sleep,
      }),
