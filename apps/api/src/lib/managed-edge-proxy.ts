...
export interface ManagedEdgeTarget {
  /** Full managed hostname, e.g. `myapp.vibrail.com` — used in log/warning text. */
  hostname: string;
...
/**
 * Sync every managed (*.vibrail.com) route for a project through the SaaS edge,
...
/**
 * Tear down every managed (*.vibrail.com) edge route for a set of freed slugs,
