...
export interface ManagedEdgeTarget {
  /** Full managed hostname, e.g. `myapp.vibrail.app` — used in log/warning text. */
  hostname: string;
...
/**
 * Sync every managed (*.vibrail.app) route for a project through the SaaS edge,
...
/**
 * Tear down every managed (*.vibrail.app) edge route for a set of freed slugs,
