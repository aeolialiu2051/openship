...
describe("public endpoint helpers", () => {
  it("rejects target paths that could escape generated routing syntax", () => {
    expect(normalizeTargetPath("/docs/v1")).toBe("/docs/v1");
    expect(normalizeTargetPath("/docs/' injected")).toBeUndefined();
    expect(normalizeTargetPath("/docs\nlocation /admin")).toBeUndefined();
  });
  ...
  it("creates a default managed route when a target path is provided", () => {
    ...
    expect(endpoints).toEqual([{ domain: "my-app", targetPath: "/docs", domainType: "free" }]);
  });
  ...
  it("keeps an explicit free port endpoint by falling back to the project slug", () => {
    ...
    expect(routing.publicEndpoints).toEqual([{ port: 3200, domain: "my-app", domainType: "free" }]);
  });
});
