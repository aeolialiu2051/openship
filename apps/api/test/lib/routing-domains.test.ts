...
describe("buildProjectRouteDomains", () => {
  ...
  it("does NOT attach the free .vibrail.com fallback when an endpoint has a custom domain", () => {
    // Regression: a self-hosted deploy with a manual/custom domain was
    // still synthesizing <slug>.vibrail.com as the primary route, which then
  ...
  it("still attaches the free .vibrail.com fallback when there is no custom domain", () => {
    const planned = buildProjectRouteDomains({
      project: { slug: "girls-collage" } as any,
      projectDomains: [],
      managedSlug: "girls-collage",
