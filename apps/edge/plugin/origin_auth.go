// Traefik's Yaegi loader derives the package identifier from the final module
// path segment by replacing '-' with '_'. Keep this exact package name: a
// conventional collapsed Go name (vibrailoriginauth) compiles with `go test`
// but fails only when Traefik evaluates New at runtime.
...
func CreateConfig() *Config {
	return &Config{
		SecretFile: "/etc/vibrail/edge/server-secret", PreviousSecretFile: "/etc/vibrail/edge/previous-server-secret",
		ManifestDirectory: "/etc/vibrail/edge-routes", TimestampSkewSeconds: 60,
	}
}
...
func normalizeHostname(value string) (string, bool) {
	...
	if len(host) == 0 || len(host) > 253 || !hostnamePattern.MatchString(host) || strings.Contains(host, "..") { return "", false }
	for _, label := range strings.Split(host, ".") {
		if len(label) == 0 || len(label) > 63 || strings.HasPrefix(label, "-") || strings.HasSuffix(label, "-") { return "", false }
	}
	return host, true
}
...
func validSignature(actual, payload string, secrets ...string) bool {
	...
	if err != nil || len(decoded) != sha256.Size { return false }
	for _, secret := range secrets {
		expected, decodeErr := hex.DecodeString(sign(secret, payload))
		if secret != "" && decodeErr == nil && hmac.Equal(decoded, expected) { return true }
	}
	return false
}
...
func (m *middleware) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if delta := time.Since(requestTime); delta > skew || delta < -skew { http.Error(w, "Forbidden", http.StatusForbidden); return }
	...
	if !validSignature(signature, payload, readSecret(m.config.SecretFile), readSecret(m.config.PreviousSecretFile)) { http.Error(w, "Forbidden", http.StatusForbidden); return }
	if !m.claimNonce(nonce, time.Now().Add(2*skew)) { http.Error(w, "Forbidden", http.StatusForbidden); return }
	manifestPath := filepath.Join(m.config.ManifestDirectory, url.PathEscape(hostname)+".json")
	raw, err := os.ReadFile(manifestPath)
	if err != nil { http.NotFound(w, r); return }
	var authority manifest
	if json.Unmarshal(raw, &authority) != nil || !authority.Enabled || strings.ToLower(authority.Hostname) != hostname || authority.ProjectID != projectID || serviceValue(authority.ServiceID) != serviceID || authority.ServerID != serverID || authority.Version < version { http.NotFound(w, r); return }
	for _, name := range internalHeaders { r.Header.Del(name) }
	// The Worker connects to server-{routingId}, but applications must observe
	// the public managed hostname for redirects, absolute URLs and cookie scope.
	r.Host = hostname
