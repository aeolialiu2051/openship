...
Pick a branch/tag with `OPENSHIP_REF` (default `main`):

```bash
OPENSHIP_REF=my-branch curl -fsSL https://get.openship.io/dev | sh
```
...
deploys apps out to a server or the cloud. To force the choice, run `openship up --compose` or `openship up --bare`.
</Callout>
...
- **Domain** — `--domain-kind`:
  ...
  - `free` — a `<slug>.vibrail.warpgateapi.com` subdomain (`--slug`). **Requires the box to already be connected to Openship
