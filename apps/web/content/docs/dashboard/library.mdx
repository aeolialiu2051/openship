...
a URL, or start from a template"*. Its job is simple: pick **where your code comes from**, and Vibrail hands
...
(**Existing server**) only appears on self-hosted and desktop installs — Vibrail Cloud has no server inventory
to adopt from.
...
- **Desktop app** — you get the **Local Projects** panel. Point Vibrail at a folder on your machine
...
- **Browser (Vibrail Cloud or a remote self-hosted instance)** — you get the **Folder upload** flow. A browser
  can't see your filesystem, so Vibrail first asks **"What are you deploying?"** and shows a grid of stacks to
...
Click any repository and Vibrail takes you straight into the deploy wizard for it.

<Callout title="'Local only' badge" type="warn">
A repo tagged **Local only** is visible through the `gh` CLI but the Vibrail GitHub App isn't installed on its
...
is required. Vibrail parses the owner and repo from the link and sends you to the wizard.

<Callout title="Public repos only">
This path clones over the public URL. For private repositories, use the **GitHub** tab so Vibrail can
authenticate. See [Deploy from a template or Git URL](/docs/guides/deploy-template-or-url).
</Callout>

## Template

A grid of framework starters. Pick a framework and Vibrail opens the wizard seeded from that template — a fast
...
Vibrail."* Press **Migrate Docker** to open the migration wizard, which connects over SSH to your box,
inspects the running containers, and brings them under Vibrail's management.

<Callout title="Needs SSH to your own machine">
Adoption reads containers off a server you control, so it isn't available on Vibrail Cloud. If you want to add
