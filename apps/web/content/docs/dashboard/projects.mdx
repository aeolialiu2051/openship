...
A **project** is one app Vibrail knows how to build and run. This is where you find every app
...
right column. It groups projects by where they run — **All projects**, **Vibrail Cloud**, each
...
new one up from a branch — pick a branch and Vibrail creates a matching environment (a `main`
...
**Route rules** apply native Traefik middleware policies to a domain or path prefix on self-hosted Docker deployments: request rate limits, IP allowlists, and in-flight request limits. Changes take effect on the next redeploy because Docker labels are part of the workload container configuration.
...
get a **record-only** option that drops Vibrail's record of the project but **leaves the running
