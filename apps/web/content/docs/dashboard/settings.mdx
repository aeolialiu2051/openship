...
The **Cloud** tab is hidden on the managed Vibrail Cloud — it shows on self-hosted and desktop installs
...
The **GitHub** card is where you link the account Vibrail reads your repositories from. If you're not
...
(local Docker/runtime), **My Server** (a configured SSH server), or **Vibrail Cloud**. The chosen target
...
This card also has **Forward my git identity to build servers** — a single per-operator switch (replacing
the old per-deploy checkbox). When on, a build that runs on a remote server clones directly on that server
using your local `gh` login, tunneled over SSH just for the clone; nothing is written to the server. When
off, the server falls back to its own git login, a stored token, the public path, or cloning on Vibrail
and transferring. See [Git authentication for private repos](/docs/guides/git-authentication) for the full
chain.
...
Connect AI agents (Codex, ChatGPT Desktop, Claude, Cursor, VS Code, Windsurf, …) to your Vibrail API over the Model Context
...
- **OAuth** — the client opens a browser to authorize; you approve access and pick what it can reach.
  Nothing to copy. Vibrail is a standards-compliant OAuth 2.1 MCP server.
...
*(Self-hosted and desktop only.)* Link this instance to Vibrail Cloud to unlock cloud deployments, managed SSL, and the
global CDN. Press **Connect to Vibrail Cloud** and sign in; once connected the card shows your cloud
...
<Callout title="Screenshot">Cloud tab in the disconnected state showing the "Connect to Vibrail Cloud" button and the features it unlocks. *(screenshot pending)*</Callout>

## Instance

Everything about *this install*. Two cards at the top show the instance type (**Desktop**, **Vibrail
...
**Update & security notifications**. Vibrail only ever *pulls* this data from the public GitHub releases —
nothing can push updates to your install. More in [Updating Vibrail](/docs/guides/updating).
