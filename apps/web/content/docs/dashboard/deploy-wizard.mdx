...
you can choose between your own servers, Vibrail Cloud, and your own machine. On a
...
<Callout title="Screenshot">The Deploy target screen: the "Where do you want to deploy?" heading with the server card(s) and the **Vibrail Cloud** card, plus the **Add your own server** button underneath. *(screenshot pending)*</Callout>
...
| **Vibrail Cloud** | Deploy to managed cloud infrastructure — no server setup. If you haven't connected Cloud yet, the card explains that and prompts you to connect when you continue. |

If you run Vibrail self-hosted, an **Add your own server** button sits below the cards.
...
server(s) and Vibrail Cloud. Running the **build** on your own computer is offered
...
When **Vibrail Cloud** is selected, a **Power** panel appears on the right. This is where
...
<Callout title="Screenshot">Vibrail Cloud selected, with the **Power** panel on the right showing the Micro / Low / Medium / High / Custom tier cards. *(screenshot pending)*</Callout>
...
- **Runtime** — how the app is isolated on the server.
  - **Docker container** (recommended) — runs in a container with process and filesystem
    isolation. Tiny overhead.
...
- **Build location** — **This Machine** (build locally, then transfer the output — faster
  ...
  is limited).
- **Clone location** — for Docker/Compose server deploys: **On your device** /
  **On Vibrail** (clone then upload; works everywhere) or **On the server** (clone on the
...
This screen is where you confirm what Vibrail detected and set anything it can't guess.
...
What renders here depends on the project type Vibrail detected:
...
Free `.<your-base-domain>` subdomains are routed through Vibrail Cloud. If you deploy to
your **own server** with a free domain (or expose a Compose service on one), the wizard
asks you to connect Vibrail Cloud or switch to a custom domain first. Custom domains work
without Cloud.
</Callout>

**Deploy Summary** — a read-only recap card at the bottom of the sidebar. For apps it lists
the **Domain**, **Build Location** (Local Machine / Vibrail Cloud / Server), **Framework**,
...
- Deploying **to Vibrail Cloud** without a Cloud connection → prompts you to connect.
...
The right sidebar summarizes the deployment: **Status**, **Instance** (Vibrail Cloud /
