...
backups get written to. Vibrail calls these **destinations**. Think of a destination as an answer to one
...
| **Path prefix** | Remote directory, e.g. `/backups/vibrail`. |
...
| **Remote path** | Absolute directory on the server, e.g. `/backups/vibrail`. |
...
| Field | Notes |
|---|---|
| **Absolute path** | A filesystem path on the API host, e.g. `/var/backups/vibrail`. Disabled in cloud mode. |
