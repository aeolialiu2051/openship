...
## Two ways to run it: Docker or direct
...
- **Docker container** — your app runs inside a **container** with its own files and
...
Docker is the default everywhere because the container isolation is worth it and the overhead is tiny. The one time
...
**nohup** on macOS — so it restarts if it crashes or the box reboots. A containerized app is kept alive by Docker.
</Callout>
...
- **Docker** talks to the machine's Docker socket directly.
...
- **Docker** drives the remote machine's Docker engine over an SSH tunnel.
...
- **Self-hosted Docker deployments** publish routing labels to the shared **Traefik** instance. Traefik's
  configured certificate resolver obtains and renews HTTPS certificates. Bare deployments require
  operator-managed public ingress.
...
- **Docker and Cloud apps can overlap.** Each version gets its own container or workspace, so Openship
