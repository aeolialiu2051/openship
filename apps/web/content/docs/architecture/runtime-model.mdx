...
The presence of a Dockerfile only decides whether Openship uses your build recipe or generates one. It does
...
Openship talks to the machine's Docker socket directly and starts the application as a container.
...
Openship drives the remote machine's Docker engine over the SSH connection. If the repository has no
Dockerfile, Openship sends a generated build recipe rather than installing the application's language
...
Docker and Cloud releases can overlap. Each version gets its own container or workspace, so Openship starts
