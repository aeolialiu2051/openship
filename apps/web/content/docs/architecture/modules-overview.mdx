...
description: A tour of the backend modules that make up the Vibrail API — what each one owns, grouped by job, with a link to its reference page.
---
...
| Module | What it owns |
|---|---|
| [projects](/docs/api/projects) | The heart of everything. A **project** is one app Vibrail manages — its source (a GitHub repo, an uploaded folder, or a local path), its environment variables, its resource limits, and its settings. This module creates and configures them. |
...
| [images](/docs/api/images) | A searchable catalog of ready-made container images (databases, tools) you can drop into a project, sourced from the Vibrail Cloud image catalog. |
...
| [github](/docs/api/github) | Connects your GitHub account and lists your repositories, branches, and files so Vibrail can deploy from them. |
...
| [mcp](/docs/api/mcp) | A **Model Context Protocol** endpoint (MCP is a standard way for AI assistants to call tools). It exposes Vibrail's actions to agents — a client connects over OAuth or with a personal access token — and every call is gated by the very same permissions. |
...
| Module | What it owns |
|---|---|
| [cloud](/docs/api/cloud) | Links a self-hosted instance to Vibrail Cloud (connect, disconnect, list cloud workspaces) and, on the SaaS side, handles the sign-in handoff between the two. |
| [billing](/docs/api/billing) | Plans, subscriptions, and usage. The public plan list runs on every instance; Stripe-backed subscription management runs on Vibrail Cloud, and a self-hosted instance proxies to it. |
...
These modules only exist on a self-hosted instance — on Vibrail Cloud they're never loaded.
