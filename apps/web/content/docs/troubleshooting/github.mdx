...
Most GitHub problems fall into one of four buckets: Vibrail can't read your repos, a **private** repo
...
Vibrail can reach GitHub three ways — the **Vibrail GitHub App**, an **OAuth** account, or the local
...
## Vibrail can't read your repositories
...
**What it means (plain words):** Vibrail has no usable credential for your account. Either you were never
connected, or a token that used to work has expired or been revoked (you signed out of `gh`, rotated a
personal access token, or removed the App). Vibrail isn't broken — it just has nothing to authenticate with.
...
3. On a self-hosted instance using the CLI fallback, run `gh auth login` in the terminal on the machine
   running Vibrail, then reload the page.

<Callout type="info">
The deploy pipeline gives a more specific hint depending on where the build runs. A local build asks you to
**"Run `gh auth login`, connect Vibrail Cloud, or set a per-project clone token in Settings."** A remote
(server / cloud) build asks you to **"Install the Vibrail GitHub App on this owner, or set a per-project
...
<Callout type="warn" title="“Vibrail Cloud is unreachable”">
If connecting returns a `503` with **"Vibrail Cloud is unreachable, so GitHub can't be connected right
now."**, your self-hosted instance is cloud-connected but can't reach Vibrail Cloud. GitHub connection
runs through the cloud in that mode, so check your network (or the cloud status) and try again — Vibrail
...
Vibrail can see it only if the **GitHub App is installed on that owner and includes that repo**, or you've
...
<Callout type="error" title="What you see">
You point Vibrail at a public GitHub repo expecting it to deploy with no login, but it still asks you to
connect a GitHub account.
</Callout>

**What it means:** A public github.com repo *can* be read and deployed with zero credentials — Vibrail
probes it anonymously first. That probe **fails closed**: if the repo doesn't come back as clearly public,
Vibrail falls back to asking for a credential. The usual reasons are that the repo isn't actually public,
...
3. If you *just* made it public, wait a minute and retry. Vibrail caches the "public" verdict for about ten
...
**What it means:** Auto-deploy is a **webhook** — GitHub sends Vibrail a message the instant you push. If
...
2. **Check the webhook is reachable.** If the Source tab shows the **"Enable auto-deploy"** banner —
   *"Auto-deploy needs a direct webhook endpoint. Expose this Vibrail API on a public URL or configure a
   verified webhook domain for direct delivery."* — then your instance is on `localhost` or a private
   network and GitHub literally can't reach it. Put Vibrail behind a public URL, or pick a
   [verified custom domain](/docs/guides/custom-domains) for direct delivery.
3. **Push the branch you actually deploy.** Vibrail only redeploys when the pushed branch matches the
...
For monorepo / multi-service projects, Vibrail only rebuilds the services whose files changed. A push that
...
If a delivery is retried, Vibrail drops the repeat (*"Duplicate delivery ignored"*) so you don't get two
