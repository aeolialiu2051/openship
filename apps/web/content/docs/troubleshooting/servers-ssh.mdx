...
When you deploy to your own machine, Vibrail reaches it over SSH — the same way you'd type `ssh you@your-box`
in a terminal. Most "my server won't work" problems come down to one of four things: Vibrail can't log in,
...
[`vibrail server`](/docs/cli/self-host).
</Callout>
...
- **Test Connection** (the button on the server form, or `vibrail server test-connection --host ...`) tries
  only the SSH login, using the details you typed, and **saves nothing**. Use it while you're still guessing at
  credentials.
- **Re-check** (the Components tab, or `vibrail server check <serverId>`) logs into a **saved** server and
...
**What it means.** Vibrail reached the server, but the server rejected the login. The network is fine — the
...
- **SSH Key** — the **Key Path** must point at a **private** key the machine running Vibrail can read
...
is on the server, not in Vibrail.
</Step>
...
**What it means.** Vibrail never got as far as logging in. Either nothing is listening on that host and port,
...
<Step>
Confirm the SSH port is open to the machine running Vibrail — check your provider's firewall / security-group
rules, not just the server's local firewall.
</Step>

<Step>
Test raw reachability from the Vibrail host: `nc -vz <host> <port>` (or `ssh -p <port> <user>@<host>`). If
that can't connect, the problem is network/firewall, not Vibrail.
</Step>

</Steps>

<Callout title="&quot;Cooling down after repeated failures&quot;" type="warn">
After a couple of consecutive failures Vibrail trips a short circuit breaker and fast-fails with
...
<Step>
Prefer configuring the bastion once in your `~/.ssh/config` and pointing Vibrail at the target host — the
...
host machine's `ssh-agent`. Vibrail looks for the agent socket in `SSH_AUTH_SOCK`, and — because a
...
<Step>
On the machine running Vibrail, confirm an agent is running and has your key:
`ssh-add -l`. If it lists "no identities", load one with `ssh-add ~/.ssh/id_rsa`.
</Step>

<Step>
Confirm plain `ssh <user>@<host>` works from that same machine. If it does but Vibrail doesn't, the agent
socket isn't being found — restart Vibrail from a shell that has `SSH_AUTH_SOCK` set, or switch to **SSH Key**
...
Vibrail installs and health-checks a small set of components on each server: **Docker**, **Git**,
**Traefik**, **Certbot**, and **rsync**. Docker and Git are the core requirements for deployments; the rest
are optional infrastructure. Manage them from the server's **Components** tab (the **System Health** card) or
with `vibrail server install`/`check`.
...
<Step>
For a missing Docker, use **Install** on the Docker row (or `vibrail server install <serverId> -c docker`).
Vibrail installs it via the official `curl -fsSL https://get.docker.com | sh` script and, on systemd hosts,
...
<Step>
Press **Re-check** (or `vibrail server check <serverId>`) — the Docker row should read healthy with a version.
</Step>

</Steps>

<Callout title="Docker installs only on Linux" type="warn">
Automated Docker (and Traefik) installation targets Linux servers. On a non-Linux host you'll see
...
<Callout type="error">
A component row shows a version but is flagged unhealthy.
</Callout>

**What it means.** The binary exists, but the health check for it failed.
...
**Remove** is destructive — removing **Traefik** takes down the reverse proxy and its managed config until
...
- `Invalid component names` / `Invalid or missing component name` — the component isn't one Vibrail installs.
  The valid names are `docker`, `git`, `traefik`, `certbot`, and `rsync`.
...
<Step>
For an invalid name, re-run with a supported component — the CLI rejects anything else up front
with `Unknown component(s): … Valid: docker, git, certbot, rsync`.
</Step>
...
sense when the Vibrail orchestrator *is* your own machine, so the feature is hidden on hosted and self-hosted
VPS instances.
...
<Step>
If the start still fails, the OS may have picked a busy local port — set **Local** to `0` so Vibrail binds any
