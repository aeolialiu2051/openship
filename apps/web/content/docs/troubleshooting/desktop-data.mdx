...
This page is for people who run Vibrail **on their own machine** — the **desktop app**, or a self-hosted
install that uses the built-in database. If you're on Vibrail Cloud, none of this applies; we run the
database for you.

<Callout title="One folder, one process" type="info">
When you don't point Vibrail at a real Postgres, it uses **PGlite** — an embedded Postgres that keeps
everything in a single folder on disk. Only **one** Vibrail process may use that folder at a time. Most of
...
## "Another Vibrail instance is already using the database"

<Callout title="Symptom / what you see" type="error">
Vibrail won't start. **Self-hosted or running from source**, the process exits on launch and your terminal
(or log) shows a message like:

> Another Vibrail instance is already using the database at `/…/data` (pid 12345). PGlite allows only one
> process per data directory; opening a second would corrupt it. Stop the other instance (e.g. quit the
> desktop app) and retry. If you are certain no Vibrail process is running, remove: `/…/data.lock`

On the **desktop app** you instead get a native dialog titled **“Vibrail failed to start”** whose body reads
...
something is already holding it. Usually that "something" is another copy of Vibrail — a second window of
...
Quit any other Vibrail you have open — a second desktop window, a terminal running `vibrail` or `bun dev`,
...
If Vibrail *crashed* last time (rather than quitting cleanly), the lock heals itself on the next start once
...
folder). If you're **sure** no Vibrail process is alive, delete that one file and start again. It's safe to
...
A different wording — *"The Vibrail database … is locked by a process on a different host … PGlite data
...
**“Vibrail failed to start”** dialog saying *"Local services failed to start after 3 attempts."*
</Callout>
...
2. **Check it isn't really the lock error.** If you also see "already using the database," fix
   [that](#another-vibrail-instance-is-already-using-the-database) first — a locked-out API never comes up,
   which is what leaves the dashboard unable to reach it.
3. **Fully quit and reopen.** Quit the app completely (not just closing the window), confirm no `vibrail`
...
it reopened straight into **“Vibrail failed to start.”**
</Callout>
...
2. **If it quit and didn't reopen, or reopened on an error:** wait a few seconds (the old version needs a
   moment to release the database), then start Vibrail yourself. If it shows the lock error, follow
   [the fix above](#another-vibrail-instance-is-already-using-the-database).
...
If the seamless self-replace can't complete, Vibrail doesn't leave you stranded — it opens the downloaded
...
Everything Vibrail stores locally sits in one place per install. Quit Vibrail before touching any of it.

| Install | Data folder |
|---|---|
| **Desktop app — macOS** | `~/Library/Application Support/Vibrail/data` |
| **Desktop app — Windows** | `%APPDATA%\Vibrail\data` |
| **Desktop app — Linux** | `~/.config/Vibrail/data` |
| **Self-hosted / CLI (default)** | `~/.vibrail/data` |
...
- **`<data-dir>.lock`** — the single-instance lock (a sibling of the data folder, never inside it). Safe to
  delete only when no Vibrail is running.
...
default `~/.vibrail/data` isn't where you want your data to live. A leading `~` is expanded for you.
</Callout>

### Recovering a corrupted database

<Callout title="Symptom / what you see" type="error">
Vibrail won't start and the logs show `RuntimeError: Aborted()`, often alongside
...
If you're on the packaged desktop app (no repo checkout) and it won't recover, quit Vibrail, **copy** your
data folder somewhere safe, then move the original aside. Vibrail recreates a fresh, empty database on the
...
<Cards>
  <Card title="Installation" href="/docs/getting-started/installation" description="Every way to run Vibrail — desktop app, npm CLI, or Docker Compose with real Postgres." />
  ...
  <Card title="Self-host infrastructure" href="/docs/cli/self-host" description="Run Vibrail as a background service on your own box." />
