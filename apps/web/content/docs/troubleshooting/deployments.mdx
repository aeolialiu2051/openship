...
A failed deploy is almost never a mystery. Vibrail checks your configuration *before* it spends any
...
**What it means** — Vibrail runs a set of *preflight* checks the moment you press Deploy, before it
...
**What it means** — when you import a repo, Vibrail inspects it and guesses the framework, package
...
<Step>
Deploy again. Vibrail remembers these values for future deploys and redeploys.
</Step>
...
<Step>
If your app **doesn't** need a build step, that's fine — but say so, so Vibrail doesn't expect a build
...
**What it means** — after starting your app, Vibrail waits up to 45 seconds for it to accept a
connection on the configured port (the default is `3000`). If nothing ever answers, either the app
crashed on startup or it's listening on a *different* port than the one Vibrail expects. This readiness
...
<Step>
Make your app bind to the port Vibrail gives it. Read the port from the environment
...
`exited with code 1`. Vibrail adds the hint: *"Build process was killed - typically because the target
...
reports only a generic non-zero exit. Vibrail scans the output for the tell-tale signs and surfaces the
real cause.
...
[Projects API resources endpoint](/docs/api/projects)). On Vibrail Cloud, pick a larger resource tier.
</Step>
...
**What it means** — when a project runs several services, Vibrail deploys each one independently. If at
...
success. Nothing is rolled back automatically — Vibrail waits for you to choose.
...
`A deployment is already in progress (dep_…). Cancel it first or wait for it to complete.` — Vibrail runs
...
failure is inside your build itself, not Vibrail. Scroll up in the build log to the first red error —
