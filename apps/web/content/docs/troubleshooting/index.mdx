...
description: Where Vibrail shows you what went wrong — the build screen, the Logs tab, and the CLI — and how to read an error before you try to fix it.
---

When something breaks, Vibrail has almost always already told you why — the trick is knowing where it
...
Whatever went wrong, scroll to the **bottom** of the output and read the **last red lines**. Vibrail (and
...
| You work from the terminal | **The CLI** | `vibrail logs <deploymentId> --follow`, plus `vibrail doctor` and `vibrail status`. |
...
vibrail logs <deploymentId>

# Stream them live until the deployment finishes
vibrail logs <deploymentId> --follow

# Just the last 50 lines
vibrail logs <deploymentId> --tail 50
```
...
`vibrail doctor` prints a checklist with a green `✓`, yellow `!`, or red `✗` next to each of **config**,
...
## How to read a Vibrail error

Vibrail errors are short and structured on purpose — they're not meant to be scary.
...
The build screen (or `vibrail logs <id> --follow`) ends on red lines instead of a live URL.
</Callout>
...
<Callout title='"Cannot reach the API at …" or a red ✗ next to "api" in vibrail doctor' type="error">
Commands hang or fail before doing anything, and `vibrail doctor` reports the **api** check as `unreachable`
...
1. Run `vibrail doctor`. It checks, in order: your **config** file, your active **context** and token, and
   whether the **api** answers.
2. If **context** is yellow with *"has no token; run `vibrail login`"*, log in again.
3. If **api** is red, confirm the instance is actually up (`vibrail status` shows its health and mode) and
