...
**verification** step where Vibrail looks those records up and confirms them. Almost every domain problem is
really one of those two records not resolving the way Vibrail expects. This page walks through the exact
...
**TXT ownership record** at host `_vibrail-challenge` (a secret code that proves the domain is yours).
Verify checks both.
</Callout>
...
is the exact reason Vibrail returned — one or both of:

- `A record not pointing to server for app.example.com` (self-hosted) or `CNAME record not found for app.example.com` (Vibrail Cloud)
- `TXT record _vibrail-challenge.app.example.com must equal "…"`

(If Vibrail can't produce a specific reason it falls back to the generic *"\{hostname} could not be verified yet."*)
...
**What it means.** Vibrail ran two independent lookups and at least one came back wrong. The message names the
...
| `CNAME record not found for …` | Routing (`CNAME`) — Vibrail Cloud | The `CNAME` is missing or doesn't point at the Cloud target. |
| `TXT record _vibrail-challenge.… must equal "…"` | Ownership (`TXT`) | The `_vibrail-challenge` TXT record is missing, or its value doesn't match the secret code. |
...
1. Open your project's **Domains** tab and reopen the **DNS Records** panel for the domain (or run
   `vibrail domain records <domainId>`).
2. Compare the routing row against what's actually at your registrar. Self-hosted expects an `A` record with
   ...
   **Vibrail Cloud target**.
3. Fix any mismatch — copy the value from Vibrail, don't retype it. Delete any old, conflicting record for the
...
1. At your registrar, add (or correct) a **TXT** record with host `_vibrail-challenge` — Vibrail looks it up
   at `_vibrail-challenge.<your-domain>`.
...
If you re-run Verify on a domain that's already done, Vibrail simply replies **"Already verified"** and
...
before Vibrail's lookup can see it. As the DNS Records panel itself notes: *"DNS changes can take up to 48
...
3. Vibrail also ships a *pending-verification sweep* (`POST /api/domains/verify-pending`) that re-checks
...
To check what the world currently sees, look the record up directly — e.g. `dig TXT _vibrail-challenge.app.example.com`
or `dig A app.example.com`. If your own lookup doesn't return the new value yet, Vibrail won't see it either;
that's a propagation delay, not a Vibrail problem.
</Callout>
...
2. If Recheck reports *"No valid certificate found for \{hostname} yet. If you just deployed, give Let's Encrypt
   ...
   again.
3. Redeploy the project after verification. The redeploy publishes the route and lets Traefik obtain TLS
   through its configured certificate resolver.

<Callout title="Renewing certificates" type="info">
On self-hosted installations, Traefik owns certificate issuance and renewal. Vibrail does not expose
application-side renew, recheck, or certificate-upload actions. If issuance fails, inspect Traefik's resolver
configuration and logs, confirm ports 80/443 and DNS are correct, then redeploy after correcting the cause.
</Callout>

<Callout title="Pending domains are intentionally not published" type="warn">
A custom hostname remains unpublished until its `_vibrail-challenge` TXT record verifies. This prevents an
unverified hostname from being advertised by Traefik. After verification, redeploy the project to publish it.
</Callout>
...
HTTPS — but Vibrail keeps asking for an `A`/`CNAME` and tries to issue its own certificate, which you don't
want.
</Callout>

**What it means.** By default Vibrail assumes it's the edge: it wants the domain to resolve to it and it
issues the certificate. When **your** edge terminates TLS, that's the wrong model — Vibrail should only
...
2. Add **only** the `TXT` record with host `_vibrail-challenge` — there's no routing record to add, because
   your edge already controls where the name points.
3. Click **Verify**. Vibrail checks the TXT alone and confirms with *"Domain verified — TLS is handled by
   your external ingress; no certificate is issued here."*

<Callout title="What the SSL pill shows" type="info">
An external-ingress domain reads **External TLS** for its SSL status — that's correct and final. Your upstream
ingress owns HTTPS; Vibrail publishes only the plain-HTTP origin route after TXT verification and redeploy.
</Callout>
...
serve plain HTTP, with no certificate from anyone. When in doubt, leave it off and let Vibrail issue the
certificate.
</Callout>
...
`"…" is not a valid public hostname.` (a bare IP, `localhost`, or a single-label name). Free `*.vibrail.warpgateapi.com`-style
