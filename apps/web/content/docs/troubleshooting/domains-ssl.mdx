...
On self-hosted installations, Traefik owns certificate issuance and renewal. Openship does not expose
...
A custom hostname remains unpublished until its `_openship-challenge` TXT record verifies. This prevents an
...
ingress owns HTTPS; Openship publishes only the plain-HTTP origin route after TXT verification and redeploy.
</Callout>
