---
title: MCP
description: Let AI clients like Claude drive your Vibrail instance as tools, over the Model Context Protocol.
---

import { Step, Steps } from 'fumadocs-ui/components/steps';

**MCP (Model Context Protocol)** is a standard way for AI clients — Codex, ChatGPT, Claude, Cursor, VS Code Copilot, and
others — to call an app's features as *tools*. Point one of those clients at Vibrail and it can list your
projects, kick off deployments, check logs, add domains, and more — using the exact same permissions you'd
have yourself.

The important part: it's not a second, looser door into Vibrail. **Every tool call re-runs the full auth
...
a per-client "add" recipe (Codex, ChatGPT Desktop, Claude Code, Cursor, VS Code, Claude Desktop, Windsurf, Zed), and the list of
clients you've already connected.
</Callout>

## The endpoint

There's one endpoint, and it's the same on self-hosted and Vibrail Cloud:
...
Vibrail is a standards-compliant **OAuth 2.1** MCP server, so capable clients need nothing but the URL.
...
```bash
claude mcp add --transport http vibrail https://<your-host>/api/mcp
```

In Codex, add the server and then start the OAuth login flow:

```bash
codex mcp add vibrail --url https://<your-host>/api/mcp
codex mcp login vibrail
```

In ChatGPT Desktop, open **Settings → Plugins → MCPs**, add an MCP server named Vibrail, select
**Streamable HTTP** as its type, and paste the endpoint as its URL. ChatGPT requires a public HTTPS endpoint;
...
The first time the client connects, a browser window opens. There you **choose what the client may do** —
read-only or full control, and either all resources or a specific set of projects, servers, and repositories.
No credential to copy or paste.
</Step>
...
```bash
claude mcp add --transport http vibrail https://<your-host>/api/mcp \
  --header "Authorization: Bearer vibrail_pat_…"
```
...
| `get_system_servers`, `get_system_servers_by_id_docker_overview` | `/api/system/servers` | [Servers guide](/docs/dashboard/servers) |
...
| `get_updates`, `post_updates_scan` | `/api/updates` | [Updating Vibrail](/docs/guides/updating) |
...
curl -s https://<your-host>/api/mcp \
  -H "Authorization: Bearer vibrail_pat_…" \
