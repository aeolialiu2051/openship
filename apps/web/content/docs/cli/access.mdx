...
the token against the same origin. A complete URL such as `HOST_DOMAIN=http://localhost:3002` keeps its
scheme. `VIBRAIL_CLOUD_API_URL` and `VIBRAIL_CLOUD_DASHBOARD_URL` can still override the two endpoints
individually and take precedence over `HOST_DOMAIN`.
