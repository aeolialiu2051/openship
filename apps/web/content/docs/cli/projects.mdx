...
description: Link a directory and manage projects, compose services, and custom domains from the terminal with vibrail init, project, service, and domain.
---
...
Log in once with `vibrail login`, and (optionally) run `vibrail init` in a project directory so deploy
commands know which project to act on. Add `--json` to any command for machine-readable output (stdout
becomes a clean data stream). Where a command wants a project, most accept a `proj_…` id; `vibrail service`
also accepts a slug or name.
</Callout>

## `vibrail init`

Links the current directory to a project by writing `.vibrail/project.json`. Later commands (like
`vibrail deploy`) read that file so you don't have to pass a project every time. With no `--project`, it
...
vibrail init --project proj_123 --environment production
```
...
sleep mode, cloud transfer, and logs. Alias: `vibrail projects`.
...
| `project login set <id>` | Configure the optional homepage + username/password card for a non-App Catalog project. |
| `project login clear <id>` | Remove the login card without deleting application env vars. |
...
| `project transfer <id> <direction>` | Promote to Vibrail Cloud or bring it back (self-hosted only). |
| `project logs <id>` | Show or stream runtime (container) logs. |
| `project server-logs <id>` | Show or stream HTTP request logs (edge / Traefik). |

### create

```bash
vibrail project create --name "My App" --slug my-app \
...
| `--slug <slug>` | Free-subdomain slug (`slug.vibrail.app`). |
...
```bash
vibrail project delete proj_123 --wipe-volumes -y
```
...
vibrail project env get proj_123 --environment production

# Upsert two secret vars and remove another (other keys are left untouched)
vibrail project env set proj_123 \
...
Use this only for a human-facing application login. The card shows the exact project homepage/login URL,
username, and password on Overview. API tokens, JWT secrets, database credentials, and encryption keys stay
in secret environment variables instead.

This command is only available for ordinary source projects. App Catalog projects use the connection outputs defined by their templates. Generate the password server-side and inject it into the service without a plaintext temporary file:

```bash
vibrail project login set proj_123 \
...
Omit `--username-env` for a fixed username. If `--generate-password` is omitted, the CLI prompts for the
password without echoing it. Remove only the Overview card with `vibrail project login clear proj_123`.
...
```bash
vibrail project git link proj_123 --owner acme --repo my-app --branch main
vibrail project git auto-deploy proj_123 --enable
vibrail project git branch proj_123 develop
vibrail project git webhook-domain proj_123 app.example.com   # or: --clear
```
...
`git link` and App auto-deploy are separate capabilities. If Vibrail can read a repository through
an available credential, linking succeeds even when the repository owner has no Vibrail GitHub App
installation; the project remains manually deployable with auto-deploy disabled. Enabling App
auto-deploy requires an owner-matching installation that covers the repository.
...
```bash
vibrail project connect proj_123 app.example.com --include-www
```
...
`project connect` and [`domain add`](#vibrail-domain) both attach a custom domain to a project and return
...
```bash
vibrail project disable proj_123
vibrail project enable proj_123
```
...
```bash
vibrail project sleep-mode proj_123 auto_sleep   # or: always_on
```
...
Moves a project between your self-hosted instance and Vibrail Cloud. Only available on a self-hosted
instance; see the [cloud connect & migration guide](/docs/guides/cloud-connect-migration).

```bash
vibrail project transfer proj_123 to-cloud        # or: to-self-hosted
```

### logs / server-logs

`logs` shows the container's runtime output; `server-logs` shows edge HTTP request logs (Traefik). Both
print recent lines by default, or stream with `-f`. More context in the
[logs & monitoring guide](/docs/guides/logs-monitoring).

```bash
vibrail project logs proj_123 --tail 100 -f
vibrail project server-logs proj_123 --limit 200 --domain app.example.com
```
...
Manage the services inside a compose **stack** (a multi-service project). Alias: `vibrail services`. Every
...
| `service sync -p <stack> <compose-file> [--replace]` | Add/update Compose services; `--replace` also removes omitted services. |
...
```bash
vibrail service create -p my-stack db \
  --image postgres:16 --port 5432:5432 --env POSTGRES_PASSWORD=secret

vibrail service create -p my-stack web \
...
```bash
vibrail service sync -p my-stack ./docker-compose.yml -y
```
...
```bash
vibrail service delete -p my-stack db -y
```
...
```bash
vibrail service drift accept -p my-stack web
vibrail service drift keep   -p my-stack web
```
...
```bash
vibrail service env get -p my-stack web --env production
vibrail service env set -p my-stack web LOG_LEVEL=debug FEATURE_X=on --env production
```
...
```bash
vibrail service logs -p my-stack web --tail 200 -f
```
...
`vibrail service exec` is stubbed — an interactive terminal needs a WebSocket client the CLI doesn't yet
...
vibrail domain preview app.example.com

# 2. Attach it to a project (prints the exact records to add)
vibrail domain add app.example.com -p proj_123 --primary

# 3. After the DNS records propagate, verify
vibrail domain verify <domainId>
```
