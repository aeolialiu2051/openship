...
Every command here talks to your instance's API and needs a token — run `vibrail login` once. The
`server`, `system`, and `mail` groups only work on a **self-hosted** instance; run against Vibrail Cloud
