...
description: CLI reference for the server, system, mail, and backup commands that operate a Vibrail instance you run yourself.
---
...
Every command here talks to your instance's API and needs a token — run `vibrail login` once. `server` and
`mail` work whenever the instance advertises the `userServers` capability, including a SaaS control plane
that orchestrates customer-owned VPS hosts. `system` remains strictly self-hosted; `backup` works on any
instance.
</Callout>
...
suppressed): `vibrail server list --json`.
</Callout>
...
| `show <id>` (alias `get`) | Show one server's non-secret connection details. |
| `reachability <id>` (alias `ping`) | Run a lightweight reachability probe. |
| `overview <id>` (alias `docker`) | Inspect live Docker workloads and running projects. |
...
vibrail server add --host 203.0.113.10 --name web-1 --user deploy --auth-method key --key-path ~/.ssh/id_ed25519
vibrail server test-connection --host 203.0.113.10 --user deploy --auth-method key --key-path ~/.ssh/id_ed25519
vibrail server list
vibrail server show srv_123
vibrail server reachability srv_123
vibrail server overview srv_123
```

### `check` and `install`

`check` reports which components are present and healthy; `install` puts the missing ones on the box.
The installable components are **`docker`, `git`, `certbot`, `rsync`**.
...
```bash
vibrail server check srv_123
vibrail server install srv_123 --component docker --component git --follow
```

```bash
vibrail server monitor srv_123                             # live stats (Ctrl-C to stop)
```
...
| `migration start-cloud` | Migrate this instance to Vibrail Cloud. |
...
```bash
vibrail system settings get
vibrail system settings set --default-build-mode server
```
...
Configure authentication email independently through Settings → Email or the
instance's `SMTP_*` environment. Hosted mail servers are not auth transports.
...
Vibrail Cloud; `start-tunnel` fronts it with an edge tunnel; `switch-back` reverses any of them.

| Command | Flag | Purpose |
|---|---|---|
| `preflight` / `start` | `--server-id <id>` | Target server id. **Required.** |
| `preflight` / `start` | `--hostname <host>` | Custom domain pointing at the server. |
| `preflight` / `start` | `--slug <slug>` | Free `<slug>.vibrail.warpgateapi.com` subdomain. |
...
```bash
vibrail system migration preflight --server-id srv_123 --hostname app.example.com
vibrail system migration start     --server-id srv_123 --hostname app.example.com
```
...
```bash
vibrail system data-transfer export --passphrase "correct horse" --out instance.json
vibrail system data-transfer import --file instance.json --passphrase "correct horse" --mode merge
```
...
```bash
vibrail mail setup srv_123 --domain example.com
# ...publish the DNS records it prints, then:
vibrail mail dns-ack srv_123
vibrail mail setup srv_123 --domain example.com --start-step 7
```
...
```bash
vibrail mail health srv_123
vibrail mail logs srv_123 postfix --lines 500
```

<Callout title="scan, adopt, forget go together" type="info">
`forget` only drops Vibrail's tracking row — the installed daemons and on-server state stay put. To pick
...
```bash
vibrail backup policy create --project proj_123 --destination dst_123 --cron '0 3 * * *' --retain-count 7
vibrail backup policy run pol_123 --follow
```
...
vibrail backup run restore run_123
vibrail backup restore apply rst_123 --token <confirmationToken> --follow
```
...
prepare step. Cancel a staged restore you no longer want with `vibrail backup restore cancel <restoreId>`.
</Callout>
...
| `--kind <kind>` | `s3_compatible`, `sftp`, `vibrail_server`, or `local`. **Required.** |
...
| `--server <id>` | Server id (for the `vibrail_server` kind). |
| `--default` | Mark as the default destination. |

```bash
vibrail backup destination create --name s3-backups --kind s3_compatible \
  --endpoint https://s3.us-east-1.amazonaws.com --region us-east-1 --bucket my-backups \
  --access-key-id AKIA... --secret-access-key ... --default
vibrail backup destination preflight dst_123
```
