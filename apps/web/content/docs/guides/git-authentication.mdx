...
description: How Vibrail authenticates the git clone when it builds your app — forwarding your local git identity, the server's own git, per-server tokens and SSH keys, and public repos.
---

import { Step, Steps } from 'fumadocs-ui/components/steps';

When Vibrail builds your app it has to **clone your repository first**. For a public repo that's free — no credentials needed. For a **private** repo, something has to prove it's allowed to read the code. This page explains every way that can happen, in the order Vibrail tries them, and how to turn each one on.

<Callout title="You usually don't have to think about this">
On Vibrail Cloud, connecting the GitHub App is all you need. On the desktop app, your local `gh` login is used automatically. This page is for the cases where a build says it **can't authenticate the clone** — almost always a private repo deploying to your own server.
</Callout>

## First, where does the clone run?

The single most important thing to understand: **a credential that works on your laptop doesn't automatically work on a remote build server.** Vibrail can clone in two places:

- **On this host** (your desktop app, or the Vibrail control plane) — then it transfers the built output to wherever the app runs.
- **On the build server itself** — the server clones directly, no upload. Best for big repos and useful when the build server already has the required credentials.

A token that lives on your machine has to be *shipped* to the server to clone there — and Vibrail is deliberately careful about which credentials it will ship. That's why the answer to "how do I auth a private clone" depends on where the clone runs.

## The credential chain (server builds)

For a build that runs off this host, Vibrail tries these in order and uses the **first** one that works. Each is independent — a step is skipped when it doesn't apply, so the order only decides who wins when several are available.
...
| 6 | **Clone on Vibrail + transfer** | Clone here, upload the source | No — the code never transits the server as a token |
...
Turn on **Settings → Clone credentials → "Forward my git identity to build servers."** When a build runs on a remote server, Vibrail forwards your local `gh` login over the SSH tunnel just for that clone. Nothing is written to the server, and it's gone the moment the clone finishes.
...
If the build server is already logged in to GitHub, Vibrail uses that — no configuration in Vibrail at all. On the server, any one of:
...
Vibrail verifies the server can actually reach *your* repo before relying on this, so a box logged in as the wrong account is skipped rather than failing mid-build.
</Step>
...
A public GitHub repo clones anonymously — no token, no forwarding, exactly like a public deploy on any platform. Vibrail confirms this by attempting the clone from the machine that will build, so a rate-limited API check never mislabels a public repo as private.
</Step>
