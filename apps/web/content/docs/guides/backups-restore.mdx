...
Vibrail does two jobs here. First, it copies your data to a storage place you choose (a **destination**).
...
- Vibrail up and running, and a project with at least one **service** that holds data (like a database).
...
Before Vibrail can back anything up, it needs to know **where** to put the copies. That place is called a
...
Vibrail asks which type of destination this is. Pick the one that matches what you have:
...
- **Existing server** — reuse the SSH login from a server you've already added to Vibrail.
- **Local disk** — write to a folder on the machine running Vibrail. (Not available in cloud mode.)
...
- **Name** and the **Server** to reuse — pick one of the servers already saved under **Servers**. Vibrail
...
- **Name** and an **Absolute path** — a filesystem path on the machine running Vibrail, like
  `/var/backups/vibrail`.

This kind is disabled when Vibrail runs in cloud mode.

</Tab>
</Tabs>

<Callout title="Your secrets are safe" type="info">
Keys and passwords are **encrypted at rest** — Vibrail stores them scrambled, not in plain text. When you
...
Vibrail writes a tiny test file, reads it back, and deletes it. A green **Verified** badge means the login and
...
- **Pre-deploy trigger** — when ticked, Vibrail also backs up automatically right before each new deploy goes
...
covers the whole project — just leave off `--service` when you run `vibrail backup policy create` (see below).
</Callout>
...
**overwrites** what's there now, so Vibrail walks you through it carefully.
...
Restoring **overwrites** the current data, and Vibrail does **not** take a fresh backup of the live state for
...
Vibrail downloads and verifies the backup at the destination to make sure it's actually usable. Nothing
...
vibrail backup policy run <policyId> --follow

# 4. See runs, and protect one from pruning
vibrail backup run list --project <projectId>
vibrail backup run protect <runId>            # or --release to unprotect

# 5. Restore — a two-step, on-purpose flow
#    'restore' stages it and prints a restoreId + confirmation token…
vibrail backup run restore <runId>
#    …then apply it with that token (this is destructive)
vibrail backup restore apply <restoreId> --token <confirmationToken> --follow
```
...
There's no one-click undo — a restore overwrites the data in place, and Vibrail doesn't auto-capture the live
