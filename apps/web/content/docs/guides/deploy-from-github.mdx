...
This is the most common thing people do with Vibrail: take some code that lives on GitHub and make it a
real, working website with its own address. Think of it like this — your code is a recipe, and Vibrail is
the kitchen that cooks it and serves the meal. You point Vibrail at the recipe once, and it does the rest.

<Callout title="What you need first">
- A Vibrail account (you're already in if you can see the dashboard).
- A GitHub repository with your app in it.
- That's it. You do **not** need Docker, a server, or any config files — Vibrail figures those out.
</Callout>
...
Vibrail needs permission to read your repository. You only do this one time.
...
Vibrail shows a summary: which framework it detected, the build command, and the web address it will use.
For most apps you don't change anything here.

Press **Deploy**.

<Callout title="Tip" type="info">
See a field you don't recognize? Leave it. Vibrail's detected defaults are correct for the large majority
...
By default you deploy when you choose to. If you'd like Vibrail to **redeploy every time you push to
...
vibrail project create --name my-app --git-owner your-name --git-repo my-app

# Ship it
vibrail deploy

# Optional: redeploy on every push
vibrail project git auto-deploy my-app --enable
```

## If something goes wrong

<Callout title="“Connect GitHub” does nothing / no repos show up" type="warn">
Your GitHub connection didn't finish, or Vibrail has no permission for that repo. Go back to
