...
When you first deploy, Vibrail gives your app a working web address for free. This guide is about the next
...
Vibrail handles the padlock for you; your only real job is adding a couple of entries to your domain's phone
book. This guide walks you through all of it.

<Callout title="What you need first">
- A project already deployed in Vibrail (see [Your first deployment](/docs/getting-started/first-deployment)).
...
- Access to that registrar's **DNS settings** — the page where you add records. That's where you'll paste
  the two lines Vibrail gives you.
</Callout>
...
domain sends traffic to. Vibrail pre-fills it with your app's port, so you can usually leave it.
...
### Copy the DNS records Vibrail shows you
...
- A **routing record** (an `A` or `CNAME` — Vibrail picks the right one for you, see below) that points the
  name at your app.
- A **TXT** record with host `_vibrail-challenge`. This is a secret code that proves you own the domain.
  Nobody else can add it, so it's how Vibrail knows the domain is really yours.

Copy each value exactly as shown — don't retype it, use the Copy button.

<Callout title="Screenshot">The DNS Records panel with the routing record and the `_vibrail-challenge` TXT record, each with a Copy button. *(screenshot pending)*</Callout>
...
Vibrail — matching the Type, Host, and Value. Save.

<Callout title="What does “Host” mean at my registrar?" type="info">
Registrars phrase it differently — you might see "Host", "Name", or "@". Paste the host exactly as Vibrail
shows it. If your registrar rejects `@`, it usually means "the root of the domain"; and a host like
`_vibrail-challenge` is added as a subdomain entry. When in doubt, copy Vibrail's value verbatim.
</Callout>
...
Back in Vibrail, click **Verify** next to the domain (it reads **Verify app.example.com**). Vibrail looks up
your DNS and checks two things: the routing record points the right way, and the `_vibrail-challenge` TXT
matches its secret code.

If both check out, the domain flips to **Verified** and Vibrail starts setting up HTTPS in the background —
...
spread across the internet. If Verify doesn't work on the first try, wait a bit and click it again. Vibrail
...
Once verified, the **SSL** status shows **Provisioning** while Vibrail requests a free certificate from
...
Vibrail figures out which records you need based on where your project runs. You don't choose the first two —
they're picked for you. The records panel tells you which one applies.

<Tabs items={['Self-hosted (A record)', 'Vibrail Cloud (CNAME)', 'External edge (TXT only)']}>
...
| `TXT` | `_vibrail-challenge` | a long verification string |

Vibrail checks that the `A` record resolves to your server and that the TXT matches, then issues the HTTPS
certificate for you.

</Tab>

<Tab value="Vibrail Cloud (CNAME)">

Your project runs on Vibrail Cloud, so the domain points at a Cloud address using a `CNAME` (a "this name is
an alias for that name" record).

| Type | Host | Value |
|---|---|---|
| `CNAME` | `@` | a Vibrail Cloud target |
| `TXT` | `_vibrail-challenge` | a long verification string |

Vibrail confirms the `CNAME` and the TXT, then handles HTTPS for you.
...
another reverse proxy in front of Vibrail. Turn on **External ingress & TLS** when adding the domain.

| Type | Host | Value |
|---|---|---|
| `TXT` | `_vibrail-challenge` | a long verification string |

Here Vibrail only checks the TXT record to confirm you own the name. It does **not** require the domain to
...
3. Vibrail serves HTTPS from that certificate and stops running Let's Encrypt for this domain.

<Callout title="Uploaded certificates don't auto-renew" type="info">
There's nothing for Vibrail to renew, so it watches the expiry and reminds you — upload a fresh certificate
before it lapses.
</Callout>
...
vibrail domain add app.example.com --project <projectId>

# Once your DNS is in place, verify it
vibrail domain verify <domainId>

# List a project's domains and their status
vibrail domain list --project <projectId>
```

<Callout title="Other domain commands" type="info">
`vibrail domain records <domainId>` reprints the records for an existing domain, `vibrail domain primary
<domainId>` makes a domain the project's main address, and `vibrail domain renew <domainId>` /
`vibrail domain verify-ssl <domainId>` manage the certificate. Full details are in the
...
Vibrail tells you which of the two checks is still missing. If the **routing record** (`A`/`CNAME`) is the
problem, it hasn't resolved to your app yet. If the **TXT** is the problem, the `_vibrail-challenge` record is
...
to 48 hours, to spread worldwide. Save the records, then wait and retry Verify — Vibrail also re-checks
