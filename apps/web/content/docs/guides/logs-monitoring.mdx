...
Vibrail gives you two kinds of diary and one big-picture view:
...
the **Terminal** sub-tab is hidden. On **Vibrail Cloud**, a static site still gets request logs, shown on a
...
Everything above is available from the CLI too. First grab your project's ID with `vibrail project list`, then:

```bash
# Runtime (container) logs — what your app prints
vibrail project logs <project-id>              # recent lines
vibrail project logs <project-id> --tail 200   # last 200 lines
vibrail project logs <project-id> -f           # follow / stream live

# HTTP request logs — every web request that hit the app
vibrail project server-logs <project-id>                 # recent entries (JSON)
vibrail project server-logs <project-id> --limit 200     # up to 200 entries
vibrail project server-logs <project-id> --domain yourapp.com   # one domain only
vibrail project server-logs <project-id> -f              # stream live

# A specific deployment's build/run log (grab the id with: vibrail deployment list)
vibrail logs <deployment-id>          # snapshot
vibrail logs <deployment-id> --tail 100
vibrail logs <deployment-id> -f       # stream until the deployment finishes
```

<Callout title="Live request streaming is self-hosted only (for now)" type="info">
`vibrail project server-logs -f` streams live on self-hosted projects. For Vibrail Cloud projects the CLI
...
on Vibrail Cloud use the **Requests** sub-tab; a self-hosted static deploy has no runtime logs to show.
</Callout>
...
(`vibrail project list` shows its status), start it if needed, then try `vibrail project logs <project-id> -f`
again.
</Callout>
