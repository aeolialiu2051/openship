...
description: Take your whole Vibrail control plane — servers, projects, services, domains, env vars and secrets — from one install to another. The classic case is a local desktop → an always-on self-hosted server.
---

import { Step, Steps } from 'fumadocs-ui/components/steps';

You started on the **desktop app** on your laptop, and now you want Vibrail itself to run on an
...
Vibrail has a first-class **data transfer** for exactly this: export your entire instance to a single
...
so Vibrail flags those projects on import. Re-point them, or just re-deploy each from a folder that lives
...
```bash
vibrail system data-transfer export --passphrase "a-strong-passphrase" --out vibrail-export.osx
```
...
```bash
scp vibrail-export.osx you@your-server:/tmp/
```
...
Install and start Vibrail on the server first (see [Installation](/docs/getting-started/installation)),
then:

```bash
vibrail system data-transfer import --file /tmp/vibrail-export.osx \
...
Run from a non-git folder, `vibrail deploy` packages the folder on the spot, uploads it to this install,
...
```bash
vibrail deploy --service-ids web,api
```
