...
ago, downloaded, or started from scratch. Vibrail can take that folder and put it online, no git remote
needed. Think of it like mailing a parcel instead of a letter: you hand over the whole box of files, and
Vibrail unpacks it, figures out what's inside, builds it, and gives you back a real web address.

<Callout title="What you need first">
- Vibrail up and running (you can see the dashboard). If not, start with [Installation](/docs/getting-started/installation).
- A folder on disk with your app in it — a website, an API, anything.
- You do **not** need Docker, a server config, or any special files. Vibrail reads your folder and works out the rest.
</Callout>

## What actually gets uploaded

Vibrail sends your **source files** — the code you wrote — and nothing else. The heavy, rebuildable stuff
...
What you see next depends on how you're using Vibrail. The **desktop app** can read a folder directly off
...
A folder picker opens. Browse to your project folder and select it — that's all Vibrail needs.
...
Vibrail shows the **Detected** stack and an editable **Project name** — press **Import Project** to add it to
...
Vibrail takes you to the deploy screen with the detected stack already filled in — the framework, the
...
In a browser, Vibrail can't see your disk — so you tell it what you're deploying, then hand it the folder
...
Vite, or Express). This tells Vibrail which build environment to prepare.
...
Vibrail packs the folder **in your browser** and uploads it, then takes you to the deploy screen. The button
...
The CLI does the whole thing in one command. When you run `vibrail deploy` in a folder that **isn't** a git
...
vibrail deploy --name my-app --watch
```

- `--name` sets the project's name (without it, the folder's name is used).
- `--watch` follows the build logs in your terminal until it finishes. Leave it off and Vibrail prints a
  command to follow the logs later.

<Callout title="Deploying inside a git repo?" type="info">
If the folder **is** a git repository, `vibrail deploy` deploys from git instead of uploading the folder.
...
Vibrail matches your folder to a project by **name**. Run `vibrail deploy --name my-app` a second time from
...
```bash
vibrail init          # pick a project to link this folder to
vibrail deploy        # from now on, deploys to the linked project
```

`vibrail init` writes a small `.vibrail/project.json` file so later commands know which project this folder
belongs to. It links to a project that **already exists**, so create one first — with a folder deploy, or in
the dashboard — then run `vibrail init` to pin future deploys to it.
...
A web browser only gives Vibrail the folder's **name**, not its full location on disk. When the form asks
