...
- For the CLI examples, the `vibrail` command installed and logged in.
- The **project ID** for the CLI/API bits. Run `vibrail project list` to find it.
</Callout>
...
- **Sleep mode** — what happens when your app is idle:
  - **Auto Sleep** (the default) — the app stops when no one is using it and wakes up on the next request.
    Cheaper to run, and Vibrail wakes it instantly, so visitors don't wait.
...
**Vibrail Cloud**. You can also set it from the API on any existing project.
...
### Start a deploy and choose Vibrail Cloud

Begin a deployment as usual. When you reach the "Where do you want to deploy?" choice, pick **Vibrail
...
<Callout title="Screenshot">The deploy screen with Vibrail Cloud selected and the **Power** section showing the Micro / Low / Medium / High / Custom size cards. *(screenshot pending)*</Callout>
...
`Authorization` header — create one from **Settings → Tokens** or with `vibrail token create`. See
...
vibrail project sleep-mode PROJECT_ID auto_sleep

# Keep it running around the clock
vibrail project sleep-mode PROJECT_ID always_on
```

Don't know your project ID? `vibrail project list` shows every project with its ID.
...
underscore), and make sure the project ID is right. `vibrail project list` confirms both.
</Callout>
...
switch back with `vibrail project sleep-mode PROJECT_ID auto_sleep` and the app will sleep when idle again.
</Callout>

## What next?

<Cards>
  <Card title="Deploy to your own server" href="/docs/guides/custom-servers" description="Run your app on a machine you control instead of Vibrail Cloud." />
