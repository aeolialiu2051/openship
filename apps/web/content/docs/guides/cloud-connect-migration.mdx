...
description: Link the copy of Vibrail you run yourself to Vibrail Cloud, move a single project up or bring it home, and (at a high level) open your instance up to a whole team.
---

import { Step, Steps } from 'fumadocs-ui/components/steps';

Vibrail can run in two places. One is **your own instance** — the copy you installed and manage yourself.
The other is **Vibrail Cloud** — the managed version, where we run the servers, SSL, and the global network
...
- Your own Vibrail instance, up and running (you can see the dashboard). The **Cloud** settings tab only
...
- A Vibrail Cloud account. You sign in — or create one — in the window that opens during the connect step, so you don't need one beforehand.
</Callout>

## Connect your instance to Vibrail Cloud
...
**Connect to Cloud** with a **Connect to Vibrail Cloud** button.

<Callout title="Screenshot">Settings → **Cloud** tab, the "Connect to Cloud" card with the **Connect to Vibrail Cloud** button. *(screenshot pending)*</Callout>
...
Press **Connect to Vibrail Cloud**. A window opens on Vibrail Cloud and asks you to sign in, then to
...
On the same card, the **Disconnect** button unlinks the account. Vibrail asks you to confirm first, because
...
This is the important boundary. **Once a project lives on Vibrail Cloud, Cloud owns it** — the cloud copy is
...
```bash
# Promote a project up to Vibrail Cloud (Cloud runs it from now on)
vibrail project transfer <project-id> to-cloud

# Bring a cloud project back to your own instance
vibrail project transfer <project-id> to-self-hosted
```

You can find a project's id with `vibrail project list`. Under the hood these call
...
- **Migrate to your server** — deploy Vibrail onto a VPS you own over SSH, and move your data there. No
  outside dependency.
- **Migrate to Vibrail Cloud** — push your data up to Cloud and let us host everything. Your local instance
  becomes a launcher that points at Cloud. (Requires the Cloud connection above.)
- **Keep local, expose via tunnel** — your data stays on this machine; Vibrail opens a public tunnel so
  teammates can reach you while your machine is online. (Also requires the Cloud connection.)

<Callout title="Screenshot">The **Move to multi-user mode** modal showing the three path cards: your server, Vibrail Cloud, and tunnel. *(screenshot pending)*</Callout>
...
vibrail system migration preflight --server-id <id> --slug myteam
vibrail system migration start     --server-id <id> --slug myteam

# To Vibrail Cloud
vibrail system migration start-cloud

# Keep local, expose via a tunnel
vibrail system migration start-tunnel --slug myteam

# Reverse any of the above, back to single-user
vibrail system migration switch-back
```

## If something goes wrong

<Callout title="There's no Cloud tab in Settings" type="warn">
The **Cloud** tab only shows on a self-hosted instance. If you're already using Vibrail Cloud, there's
...
<Callout title="“This organization is not connected to Vibrail Cloud”" type="error">
...
The sign-in or consent step didn't finish. Press **Connect to Vibrail Cloud** again and complete both the
