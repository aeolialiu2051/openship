...
description: Add one of your own machines over SSH and let Vibrail install everything it needs to run your apps.
---

import { Step, Steps } from 'fumadocs-ui/components/steps';

Vibrail can run your apps on a machine you already own — a cheap VPS, a spare box, a rented cloud
server. You hand Vibrail the address and a way to log in over **SSH** (the standard secure way to
...
- A **self-hosted** Vibrail (connecting your own servers is a self-hosted feature — on Vibrail Cloud we run the machines for you).
...
- That's it. You do **not** need to pre-install Docker or anything else — Vibrail installs what's missing.
</Callout>
...
<Callout title="Screenshot">The Servers page empty state, with the **Add Server** button and the "What gets configured" cards (Docker, Traefik, Monitoring, Git). *(screenshot pending)*</Callout>
...
- **Server Name** — an optional label like `Production` or `Staging`, just so you can tell servers apart. Leave it blank and Vibrail uses the IP address.
...
- **Authentication** — pick one:
  ...
  - **Agent** — use the SSH agent already running on the machine that hosts Vibrail (like VSCode does). No password or key needed here, but that host must already be allowed to log in to the server.
...
Press **Test Connection** first. Vibrail tries to log in and reports back without saving anything. A
...
Once the server is saved, Vibrail offers two ways to install what's needed:

- **Automatic Setup** — one click. Vibrail checks the server and installs everything missing for you.
...
Vibrail now connects over SSH and installs what your apps need to run. You'll see each piece light up
...
The routing and certificate tools aren't installed now. Vibrail adds them **automatically later**, the
first time a deployment actually needs them — you never install them by hand:

- **Traefik** — the reverse proxy that routes web traffic to the right app. Added the first time an app needs a public web address.
...
- **Security** — a read-only **port-exposure scan** that lists which ports are listening and flags any bound to a public interface vs loopback-only.
- **Terminal** — a live SSH shell right in the browser, so you can poke around the server without leaving Vibrail. Up to 3 shells at once.
...
If you run Vibrail as the **desktop app**, the **Ports** tab lets you reach a port on the remote
...
Everything above works from the CLI. You must be logged in (`vibrail login`) and on a self-hosted
Vibrail.
...
vibrail server install <server-id> --component docker git --follow
```

You can also test a connection without saving it or watch live stats:

```bash
# Try credentials without creating a server entry
vibrail server test-connection --host 203.0.113.10 --user root --auth-method key --key-path ~/.ssh/id_ed25519

# Stream live CPU / memory / disk stats (Ctrl-C to stop)
vibrail server monitor <server-id>

```

The general server components are `docker`, `git`, and `rsync`. Feature-specific installers may add their own dependencies.

## If something goes wrong

<Callout title="Can't reach the server" type="error">
Vibrail couldn't open an SSH connection. Check that the machine is powered on, that its SSH service
is accepting connections on the port you entered (usually `22`), and — if it's behind a firewall or
NAT — that the host is reachable from where Vibrail is running. The server page shows this as a
**Can't reach** banner with the exact host and port.
</Callout>

<Callout title="SSH credentials rejected" type="warn">
Vibrail reached the server but the login was refused. The key or password you saved doesn't match
what the server allows in its `~/.ssh/authorized_keys`. Open **Edit** on the server and fix the
credentials, then run a health check again. If you chose **Agent**, make sure the host running Vibrail
...
<Cards>
  <Card title="Deploy from GitHub" href="/docs/guides/deploy-from-github" description="Put an app on your new server — choose 'Server' when Vibrail asks where to run it." />
