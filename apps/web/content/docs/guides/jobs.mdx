...
description: Run maintenance on a schedule — the built-in system jobs Vibrail manages for you, plus your own scheduled commands.
---

Jobs are tasks that run on a **schedule** on your instance: the maintenance
Vibrail does for you automatically, and any commands you want to run yourself.
Find them in the dashboard under **Jobs**.

<Callout title="Self-hosted only" type="info">
Jobs live on a self-hosted instance. On Vibrail Cloud the platform handles this
...
Vibrail ships a set of system jobs and keeps them running for you — nothing to
set up:
...
part of Vibrail) — disable it instead.
...
Vibrail validates the cron when you save and shows the **next run time**.
