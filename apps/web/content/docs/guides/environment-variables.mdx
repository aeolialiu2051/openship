...
them as sticky notes you hand your app: `DATABASE_URL = ...`, `STRIPE_KEY = ...`. Vibrail keeps those notes
...
- A project you've already created in Vibrail (if not, start with [Deploy from GitHub](/docs/guides/deploy-from-github)).
- The names and values you want to set — for example `DATABASE_URL` and its connection string.
- Nothing else. Vibrail encrypts and stores the values for you.
</Callout>
...
Whether or not you mark a variable secret, Vibrail **encrypts every value before saving it**. Marking a
...
Press **Save changes**. Vibrail only writes the rows you actually added, changed, or deleted — every other
...
something is already live, Vibrail offers a shortcut to push the change out immediately — see
...
Vibrail hands your variables to your app at **two moments**:
...
the dots. To keep it, leave it alone — Vibrail keeps the stored value untouched.

## Production and preview values

Vibrail keeps variables **per environment**, so your test setup and your live setup can differ (a `preview`
...
Press it and Vibrail recreates the running container from the image it already built, with the new values
...
The same thing from the terminal is `vibrail deploy --refresh` (shown below).
...
vibrail project env get <project-id>

# Set one or more values (repeat --set for each)
vibrail project env set <project-id> --set DATABASE_URL=postgres://... --set LOG_LEVEL=info

# Mark the values in this command as secret
vibrail project env set <project-id> --set STRIPE_KEY=sk_live_... --secret

# Remove a variable
vibrail project env set <project-id> --unset OLD_FLAG

# Target a different environment (defaults to production)
vibrail project env set <project-id> --environment preview --set API_URL=https://staging.example.com

# Push the current env to the running app — no git pull, no rebuild
vibrail deploy --refresh
```
...
vibrail service env get web -p my-stack

# Set values for a service (KEY=VALUE pairs)
vibrail service env set web -p my-stack DATABASE_URL=postgres://... --secret

# Pick the environment (defaults to production)
vibrail service env set web -p my-stack -e preview API_URL=https://staging.example.com
```
...
rebuild)** (or `vibrail deploy --refresh`) to push it out now, or just redeploy.
</Callout>

<Callout title="A secret shows as dots and I can't read it back" type="info">
That's on purpose — Vibrail never reveals a stored secret's value again, in the dashboard or the CLI. If you
...
Your app expected a variable that isn't set yet. Add it in the **Configuration** tab (or with `vibrail
