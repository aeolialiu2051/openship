...
Add each custom domain, publish its `_openship-challenge` TXT record, and verify ownership. A subsequent
...
Openship exposes a focused Traefik rule surface: **per-route rate limiting, IP allowlists, and in-flight
