...
- A project in Vibrail that has been deployed at least once.
...
- **Snapshotted** — Vibrail still has this version's finished build saved, so you can roll back to it fast, without a rebuild.
...
signal Vibrail still has its build ready to restore. (A **Pinned** row works too: pinning keeps that saved
...
Press **Rollback to this version**. Vibrail asks you to confirm, and reminds you what rollback does:

> This restores the application code only. Database and volume data are unchanged.

Confirm, and Vibrail switches the live site back to that version.
...
Rolling back does **not** rebuild anything. Vibrail kept a copy of that version's finished build, so it
simply switches back to it. Two things follow from that:

- **It's fast, with no long outage.** There's no waiting for a fresh build — Vibrail restarts the saved
...
Rollback needs the saved build (the **Snapshotted** label). Vibrail only keeps a limited number of old
...
On the **Deployments** tab, press **Redeploy Project**. Vibrail pulls the latest commit on your deploy
...
`vibrail deployment redeploy <id> --use-existing-commit` (below).
</Callout>

## Pin a version so it's always there

Vibrail keeps only so many old builds around, then cleans up the oldest to save space. If there's a
...
vibrail deployment redeploy <deployment-id> --use-existing-commit

# Pin a deploy so it's never cleaned up (add --off to unpin)
vibrail deployment pin <deployment-id>
vibrail deployment pin <deployment-id> --off
```

<Callout title="Tip" type="info">
To simply ship the latest commit of the whole project (the terminal equivalent of **Redeploy Project**), run
`vibrail deploy` from the project folder. Use `vibrail deployment redeploy` when you want to re-run one
