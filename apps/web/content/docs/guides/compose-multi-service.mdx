...
three separate stations, but together they're one restaurant. Vibrail calls a set of services that run
...
- A project in Vibrail. A stack is just a project that has more than one service.
...
If your repo has a compose file, Vibrail reads it and turns **each service in the file into a service row** —
its image (or a build context and Dockerfile), ports, volumes, environment, command, restart policy,
`depends_on` links, and healthcheck. From then on those rows are what Vibrail deploys.

Vibrail tracks the compose fields it models: `image`, `build`, `dockerfile`, `ports`, `depends_on`,
`environment`, `volumes`, `command`, `restart`, and `healthcheck`. Everything else in your file — `networks`,
build args, labels, `profiles`, comments — is left untouched: Vibrail neither reads it nor rewrites it.

<Callout title="Vibrail never writes to your repo" type="info">
Edits you make in the dashboard are stored by Vibrail and used on every deploy. Your `docker-compose.yml` is
...
Vibrail pre-fills what it can. Check these:
...
You don't normally sync by hand — on every deploy (and on every push, for GitHub-backed projects) Vibrail
...
```bash
# Add or update service rows from a compose file. Omitted rows are preserved.
vibrail service sync --project my-app ./docker-compose.yml

# Replace the complete stack. Services absent from the file are removed.
vibrail service sync --project my-app ./docker-compose.yml --replace
```

<Callout title="Sync needs Docker Compose v2" type="warn">
`vibrail service sync` shells out to `docker compose config` to parse and validate the file, so Docker
...
- **Overview** — read-only facts, each shown only when it applies: **Network** (the internal address other
  ...
  container ID — shown as a **Workspace ID** on Vibrail Cloud), **Configuration** (restart policy, command,
...
<Tab value="CLI">
```bash
vibrail service start   --project my-app db
vibrail service stop    --project my-app db
vibrail service restart --project my-app db

# See what's actually running
vibrail service containers --project my-app
```
...
On each deploy Vibrail does a **three-way compare** for every service:
...
| Repo changed? | You edited it? | What Vibrail does |
| --- | --- | --- |
...
vibrail service drift accept --project my-app web

# Keep your edits and stop flagging the change
vibrail service drift keep --project my-app web
```
</Tab>
...
- **Vibrail Cloud:** only the **container** port matters — it's routed to your domain through the edge
...
```bash
vibrail service list    --project my-app                 # list the services
vibrail service get     --project my-app db              # show one service's config
vibrail service create  --project my-app cache \
  --image redis:7 --port 6379                             # add a service by hand
vibrail service sync    --project my-app ./docker-compose.yml
vibrail service start   --project my-app web             # start / stop / restart
vibrail service logs    --project my-app web --follow    # stream logs
vibrail service env set --project my-app web KEY=value   # set env vars (--secret, -e <env>)
vibrail service drift accept --project my-app web        # resolve drift
vibrail service delete  --project my-app cache           # remove a service
```
...
deployment, Vibrail redeploys for you; if it didn't, open the project and deploy it (or use the service's
**Enable & start** button on its Settings tab).
</Callout>

<Callout title="`vibrail service sync` says docker wasn't found" type="error">
...
<Cards>
  ...
  <Card title="Runtime model" href="/docs/architecture/runtime-model" description="The three places a stack can run — this machine, your server, or Vibrail Cloud." />
