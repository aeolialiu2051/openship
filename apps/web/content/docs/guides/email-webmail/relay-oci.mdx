...
description: Route your Vibrail mail server's outbound mail through Oracle Cloud (OCI) Email Delivery — free tier, good IP reputation, no PTR chores.
---
...
- An **Vibrail mail server** already set up. If you haven't done that yet, start with
...
### Point Vibrail's relay at OCI

In Vibrail, go to **Emails → Sending → "Relay via Amazon SES (or any SMTP relay)"** and choose provider
**Custom**. Fill in:
...
The same host, port, and credential also work under **Settings → Email**, if you'd rather relay Vibrail's own
...
<img src="/docs/relay-oci-form.png" alt="Vibrail Emails -> Sending relay form configured for OCI Email Delivery (provider Custom, port 587)" width="720" />
