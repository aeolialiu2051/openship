...
Vibrail's mail relay handles it natively — go to **Emails → Sending → "Relay via Amazon SES (or any SMTP
relay)"** and choose provider **Custom** for any relay that doesn't have a preset. The same host, port, and
credential fields also work under **Settings → Email**, if you'd rather relay Vibrail's own notification emails
...
- **Host and port** to plug into Vibrail's relay form — almost always port `587`.
...
same **Emails → Sending** form. If you've set one of these up with Vibrail, a PR adding a page here for your
provider is welcome.
