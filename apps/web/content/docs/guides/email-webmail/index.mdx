---
title: Email & webmail
description: Run your own self-hosted mail server and a webmail app with Vibrail — provision it, manage domains and mailboxes, and put an inbox in the browser.
---

import { Step, Steps } from 'fumadocs-ui/components/steps';

Most people rent their email from someone else — Gmail, Outlook, and the like. Vibrail lets you run the
...
Vibrail keeps them all in a single **Mail servers** list — provision as many as you like and switch between
...
Email lives entirely in **self-hosted Vibrail**. The **Emails** item only appears in the sidebar when you're
running Vibrail on your own infrastructure — it isn't part of the managed cloud. If you don't see it, you're
on a hosted plan. See [Installation](/docs/getting-started/installation) to self-host.
</Callout>

<Callout title="What you need first">
- Vibrail running self-hosted, with at least one **connected server** (a clean Ubuntu or Debian machine is
...
- **Mail server** — the software that actually sends and receives email. Vibrail installs a well-known stack
...
server** to start another. (With a single existing server, Vibrail opens it straight to its admin panel; use
...
Choose which of your connected servers the mail stack should run on. If you only have one, Vibrail picks it
for you.

Then fill in two things:

- **Domain** — your bare domain, like `example.com` (no `https://`, no path). Vibrail shows you that your
...
Click **Start Setup**. Vibrail connects to your server and works through the install one step at a time
...
Vibrail finishes the SSL step and the page flips to the **admin panel**. Your mail server is live. It also
...
<Callout title="Already have a mail server from Vibrail?" type="info">
If you set one up before and Vibrail lost track of it (for example, you moved to a new computer), you don't
...
When you add a domain under the **Domains** tab, Vibrail generates its DNS records (including a fresh DKIM
...
A mail server on its own has no web page — you'd point a mail app at it. Vibrail can also deploy **Zero**, a
...
- **Deploy to** — where the webmail app should run. You can put it on **this mail server**, on **another one of
  your servers**, or (where offered) on the managed **Vibrail Cloud** option.
...
Click **Deploy webmail**. Vibrail creates a project and a deployment for you and drops you on the standard
...
Email is self-hosted only. It appears when Vibrail is running on your own infrastructure. On a managed/cloud
