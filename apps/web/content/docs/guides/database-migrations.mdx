...
description: Safely back up and migrate a persistent database (MySQL/Postgres) volume around a deploy — using Vibrail's pre-deploy backup trigger and post-deploy jobs.
---
...
Vibrail gives you two building blocks to make this safe — a backup *before* the deploy and a migration
...
```bash
vibrail deploy --service-ids api,web
```
...
every deploy, Vibrail runs that backup **after the new build succeeds but before the destructive cutover**,
