...
project online, or spin up a fresh app from a blank starter. Vibrail has two shortcuts for exactly that,
...
checkout desk — Vibrail's normal setup screen.
...
- For the **Template** path: nothing at all — Vibrail provides the starter code.
...
This box reads **public** repositories on **github.com**. You don't need to connect anything — Vibrail
...
Vibrail reads the repo and drops you on the same setup screen as any other deploy: it shows the framework
...
No code yet? A template is a ready-made starter app that Vibrail hosts for you. Pick a framework and you
...
Click the tile for the stack you want. That's the whole choice — Vibrail uses its matching starter as the
...
```bash
# For https://github.com/vibrail/example →  owner = vibrail, repo = example
vibrail project create --name my-app --git-owner vibrail --git-repo example
...
Vibrail couldn't find an owner and repository in what you pasted. The box only accepts **github.com**
