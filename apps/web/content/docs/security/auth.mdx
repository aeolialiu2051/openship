...
description: How you prove who you are to Vibrail — session cookies, personal access tokens, MCP OAuth, and desktop loopback mode.
---

import { TypeTable } from 'fumadocs-ui/components/type-table';

Every request to Vibrail has to answer one question first: **who are you?** Vibrail accepts four
...
| **Personal access token (PAT)** | `Authorization: Bearer vibrail_pat_…` | The CLI, scripts, server-to-server, and MCP clients without OAuth |
...
A single request uses exactly one of these. Vibrail checks for a `Bearer` token first; if there isn't
...
When you log in to the dashboard, Vibrail gives your browser a **signed, httpOnly session cookie**.
...
The cookie name is prefixed with your instance mode — `vibrail.session_token` on a self-hosted install,
`vibrail-cloud.session_token` on Vibrail Cloud.
...
browser origins, Vibrail rejects it (`401 BEARER_NOT_ALLOWED_FROM_BROWSER`). Bearer tokens are for
...
```
Authorization: Bearer vibrail_pat_…
```

Every token looks like `vibrail_pat_` followed by a 43-character random secret (256 bits of entropy).
Vibrail stores **only a SHA-256 hash** of it — the plaintext is shown to you exactly once, at creation,
...
vibrail token create "my laptop"

# A read-only token that expires in 90 days
vibrail token create "ci-readonly" --read-only --expires 90

# A token scoped to one project (read + write only on that project)
vibrail token create "deploy-bot" --grant project:proj_123:read,write

vibrail token list      # your tokens (never shows secrets)
vibrail token revoke <id>
```

<Callout title="Use a token from the CLI" type="info">
`vibrail login` stores a token so the CLI can send it on every call. Run it with `--token vibrail_pat_…`
...
Vibrail is a standards-compliant **OAuth 2.1** server for the **Model Context Protocol (MCP)** — the
...
1. **Not CLI-managed or public.** Zero-auth is for the **desktop app only**. An instance started from the
   CLI (`vibrail up` sets `VIBRAIL_REQUIRE_AUTH`) or served publicly (`--public-url` /
   `VIBRAIL_PUBLIC_URL`) refuses zero-auth outright — it always requires a login.
2. **Opt-in.** The instance's auth mode must be `none`. This is the default for the desktop app; on any
   other deployment it also requires the operator to set `VIBRAIL_ALLOW_ZERO_AUTH=true`.
...
`VIBRAIL_ALLOW_ZERO_AUTH=true` — and even then the loopback check keeps it usable only from the machine
...
Vibrail recognizes three auth modes overall:
...
- **`local`** — standard login required (email + password via [Better Auth](https://www.better-auth.com/)).
  Default for a fresh self-hosted install, and always used for CLI (`vibrail up`) and public instances.
- **`cloud`** — the desktop app signs in with a Vibrail Cloud account instead of a local one.
...
`vibrail` guided setup, over an internal-token-gated, one-shot endpoint that a browser can't reach. After
...
Choosing **Continue with Cloud** (or connecting a self-hosted instance to Vibrail Cloud) runs a PKCE
...
Vibrail keeps secrets where they're safest instead of copying them around:
...
- The **GitHub App private key** lives only on Vibrail Cloud; self-hosted instances mint short-lived App
  tokens through the cloud rather than holding the key.
- Per-project git tokens and environment secrets are encrypted at rest.

## Revocation

- **A token:** `vibrail token revoke <id>`, or the **Revoke** button in Settings → Tokens. It stops
...
<Cards>
  ...
  <Card title="The local ↔ cloud boundary" href="/docs/security/cloud-boundary" description="How self-hosted instances talk to Vibrail Cloud without leaking identity." />
</Cards>
