...
The GitHub API connects a Vibrail instance to GitHub — via the Vibrail GitHub App, an OAuth account,
...
[`vibrail token create`](/docs/cli/access). The dashboard uses your session cookie instead. See the
...
| `POST /api/github/repos/:owner/:repo/webhooks` | `github:write` | Register (or reuse) the Vibrail push webhook. |
...
the "Connect Vibrail GitHub App" and "Use gh CLI" buttons; omit it to let the server pick based on the instance's
auth mode.
...
Body (or `?source=`): `source` is `"oauth"`, `"cli"`, or `"all"` (default `"all"`). This drops Vibrail's
stored credentials for that source; it does **not** uninstall the GitHub App — that happens only when you
remove it from github.com (Vibrail reacts to the uninstall webhook).

```bash
vibrail api /github/disconnect -X POST -d '{"source":"cli"}'
```
...
```bash
vibrail api /github/repos -X POST -d '{"name":"my-app","private":true}'
```
...
```bash
# Repos for one owner
vibrail api /github/repos --query owner=acme

# Branches for a repo
vibrail api /github/repos/acme/my-app/branches

# One repo, with its branches inlined
vibrail api /github/repos/acme/my-app --query branches=true
```
...
`POST /api/github/repos/:owner/:repo/webhooks` registers the Vibrail push webhook on the repo (or reuses an
...
```bash
vibrail api /github/repos/acme/my-app/webhooks -X DELETE -d '{"hookId":123456}'
```
...
the build pipeline clones with, and is available only when the Vibrail GitHub App is installed for the owner.
...
installation token, so this route 409s. Install the Vibrail GitHub App on that account to use clone tokens.
</Callout>

<Callout title="503 — cloud_unreachable" type="error">
On a self-hosted instance connected to Vibrail Cloud, GitHub OAuth and App-install URLs are resolved
through vibrail.warpgateapi.com. If the SaaS is unreachable, `connect` returns 503 rather than a dead install link. Check
