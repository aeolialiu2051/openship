---
title: Migration API
description: Scan a server's existing Docker deployment and adopt the discovered services as a Vibrail project.
---

import { TypeTable } from 'fumadocs-ui/components/type-table';

The Migration API brings workloads you deployed to a server by hand — Compose stacks and stray containers —
under Vibrail management. It inspects a server's Docker over SSH, returns the stacks it can adopt, and then
creates a Vibrail `services` project from the ones you pick. In the dashboard this is the **migration
...
into a server you manage, which Vibrail Cloud has no notion of. On a cloud instance these routes are not
...
| `POST /api/migration/adopt` | `server:write` | Create a Vibrail project from the selected discovered services. |

Both routes take the target `serverId` in the request body (not the URL). Vibrail checks that the server
...
Creates a Vibrail `services` project from the services you selected. The server is re-discovered
...
<TypeTable
  type={{
    ...
    projectName: { type: 'string', description: 'Name for the new Vibrail project.', required: true },
...
[`vibrail server list`](/docs/cli/projects) to get valid IDs. On a cloud instance the module isn't mounted
at all, so every path here returns `404`.
</Callout>

<Callout title="502 — scan or adopt failed" type="error">
Vibrail couldn't reach or inspect the server's Docker (SSH unreachable, Docker not running, permission
