...
The System API runs the machinery behind a self-hosted Vibrail box: first-run onboarding, the SSH
**servers** you deploy onto, installing the components those servers need (Docker and Git),
...
the **Settings → Servers** area; from the terminal it's [`vibrail server`](/docs/cli/self-host) and
[`vibrail system`](/docs/cli/self-host).
...
cloud runtime). Every path below is unavailable on Vibrail Cloud.
</Callout>
...
[`vibrail token create`](/docs/api/tokens). The dashboard uses your session cookie instead. See the
...
| `POST /api/system/migration/start-cloud` | `settings:admin` | Migrate → Vibrail Cloud. |
...
```bash
vibrail system onboarding apply --ssh-host 203.0.113.10 --ssh-user root --ssh-auth-method agent
```
...
vibrail system settings set --default-build-mode docker
```
...
as admin. The operator must **both** start the process with `VIBRAIL_ALLOW_ZERO_AUTH=true` (else **403**) and
...
Authentication and other system email uses Settings → Email or the instance's
`SMTP_*` environment. User-hosted mail servers are never selected as auth-mail
transports.

```bash
vibrail system upgrade-to-auth --name "Ada" --email ada@example.com
```
...
```bash
# Same thing from the CLI:
vibrail server add --host 203.0.113.10 --user root --auth-method agent
```
...
<TypeTable
  type={{
    ...
    component: { type: 'string', description: 'A single installable component name (docker, git, certbot, or rsync). Validated against an allowlist.', required: true },
...
```bash
# One-shot install:
vibrail server install <serverId> --component docker

# Stream logs live (attaches to POST /api/system/install/stream):
vibrail server install <serverId> --component docker --component git --follow
```
...
disk, uptime, and load average gathered over SSH — `vibrail server monitor <serverId>`.
...
```bash
vibrail system browse /srv/apps
```
...
- `POST /api/system/migration/start-cloud` — Path B, migrate to Vibrail Cloud. Body: `{ allowNonEmptyTarget?: boolean }`.
...
vibrail system migration preflight --server-id <id> --hostname app.example.com
vibrail system migration start --server-id <id> --hostname app.example.com
```
...
```bash
vibrail system data-transfer export --passphrase "$SECRET" --out instance.json
vibrail system data-transfer import --file instance.json --passphrase "$SECRET" --mode wipe
```
