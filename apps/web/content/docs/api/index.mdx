---
title: API overview
description: The Vibrail HTTP API — base URL, authentication, permissions, rate limits, and the standard JSON error shape.
---

Everything the dashboard and the [`vibrail`](/docs/cli) CLI can do is a call to the same HTTP API: create
...
hosted `/v1` gateway and no separate key format: the base URL is wherever you run Vibrail. Authenticate with a
personal access token as a bearer header (`Authorization: Bearer <token>`), created with
[`vibrail token create`](/docs/cli/access); the dashboard uses a session cookie instead. Full details in
[Authentication](/docs/security/auth).
</Callout>

## Authentication

A request proves who it is in exactly one of four ways. Vibrail checks for a `Bearer` token first, then a
session cookie, then the loopback fallback (only if that mode is enabled).

| Method | What you send | Who uses it |
|---|---|---|
| **Personal access token** | `Authorization: Bearer vibrail_pat_…` | CLI, scripts, server-to-server |
...
Access is decided **per organization**. Vibrail resolves which org you mean from the `X-Organization-Id`
header (falling back to your session default), then checks your role there — `owner`, `admin`, `member`, or
`restricted` (grant-only). Vibrail refuses to boot if any route is missing a tag, so there is no accidentally
...
`VIBRAIL_PUBLIC_URL` set, requests arriving over loopback skip the ordinary limits — the `auth-tight` login
...
<Cards>
  ...
  <Card title="Cloud" href="/docs/api/cloud" description="Link an instance to Vibrail Cloud and its control-plane endpoints." />
