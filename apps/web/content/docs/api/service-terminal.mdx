...
deployment modes: on self-hosted instances it's a `docker exec` into the container; on Vibrail Cloud it's
...
[`vibrail token create`](/docs/cli/access); the dashboard uses your session cookie instead. The WebSocket
...
project. A service you can't administer (or that isn't in your active organization) returns `404` — Vibrail
...
There's no working CLI shell yet — `vibrail service exec` is a stub that prints "coming soon" (the CLI
...
- **Identity** — pass the ticket from `/ticket` as a WebSocket **subprotocol**, prefixed with
  `vibrail.terminal.v1+` (e.g. `vibrail.terminal.v1+<token>`). If no ticket is present, Vibrail falls back
  to the session cookie (dashboard case). To resume a parked session, add a second subprotocol prefixed with
  `vibrail.terminal.resume+<resumeToken>`.
...
websocat "wss://your-host/api/services/terminal/ws/svc_123" \
  --protocol "vibrail.terminal.v1+$TICKET" \
