---
title: Health API
description: Public liveness and deployment-info probes for load balancers, Docker health checks, and clients discovering how an instance is configured.
---

import { TypeTable } from 'fumadocs-ui/components/type-table';

The Health API is how anything outside Vibrail checks that an instance is alive and learns how it's set
...
From the terminal it's the [`vibrail status`](/docs/cli/run) command. Both routes are available on every
...
(`Authorization: Bearer <token>`, created with [`vibrail token create`](/docs/cli/access)); the dashboard
...
<TypeTable
  type={{
    ...
    authMode: { type: 'string', description: 'Which login flow to use: "none" (zero-auth local user), "cloud" (external auth on Vibrail Cloud), or "local" (local Better Auth).' },
    ...
    cloudAuthUrl: { type: 'string', description: 'Cloud dashboard URL this instance targets (respects VIBRAIL_CLOUD_TARGET).' },
    cloudApiUrl: { type: 'string', description: 'Cloud API URL this instance targets (respects VIBRAIL_CLOUD_TARGET).' },
    machineName: { type: 'string', description: 'Friendly local machine name. Desktop only — omitted otherwise.' },
    siteDomain: { type: 'string', description: "The Vibrail site/control-plane domain." },
    managedDomain: { type: 'string', description: "The base domain used for generated deployment hostnames." },
  }}
/>
...
(wrong host/port, container still starting, or the instance is down). `vibrail status` reports this as
