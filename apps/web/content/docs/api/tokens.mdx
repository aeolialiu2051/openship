...
The Tokens API manages the credentials that authenticate you to Vibrail: **personal access tokens** (PATs)
...
[`vibrail token`](/docs/cli/access) command. Every route is self-scoped — you only ever see and manage your
...
The response's `data.token` (an `vibrail_pat_…` string) is shown once and never again. `GET /api/tokens` only
...
```bash
vibrail token revoke tok_abc123
```
...
When an AI assistant connects to Vibrail over OAuth, the consent page records what that client is allowed to
do, then Vibrail issues it scoped access tokens that run through the **same** scoped-principal path as a PAT.
...
```bash
vibrail api /tokens/mcp-clients
```
...
```bash
vibrail api /tokens/mcp-clients/<clientId> -X DELETE
```
