---
title: Settings API
description: Read and update your Vibrail workspace preferences — build mode, deploy defaults, and git clone credentials.
---
...
and sync to Vibrail Cloud. In the dashboard this is the **Settings** page; there is no dedicated `vibrail`
CLI command for these values (the CLI's `vibrail system settings` manages instance-level settings, a
...
gets `403`. Mint one with the right scopes via [`vibrail token create`](/docs/cli/access), or see
