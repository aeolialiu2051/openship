---
title: Projects API
description: Create and configure projects, link a git repo, manage env vars and resources, read logs, deploy an uploaded folder, and transfer projects to and from Vibrail Cloud.
---

import { TypeTable } from 'fumadocs-ui/components/type-table';

A **project** is the unit Vibrail builds and runs: a source (a git repo, a local path, or an uploaded
...
[`vibrail projects`](/docs/cli/projects) command.
...
The Projects API is served on both self-hosted instances and Vibrail Cloud. On a self-hosted instance,
...
| `POST /api/projects/:id/routing/retry` | `project:write` | Retry DNS propagation and live service/Traefik routing without rebuilding containers. |
...
| `POST /api/projects/:id/transfer/to-cloud` † | `project:admin` | Promote a local project to Vibrail Cloud. |
...
<TypeTable
  type={{
    ...
    slug: { type: 'string', description: 'Override the auto-generated slug (used as the free subdomain slug.vibrail.warpgateapi.com).' },
...
[Deployments API](/docs/api/deployments) (`POST /api/deployments`) or `vibrail deploy`.
</Callout>
...
```bash
vibrail projects update proj_123 --build-command "pnpm build"
```
...
```bash
vibrail env set DATABASE_URL="postgres://…" --project proj_123 --secret
```
...
Promote a local project to Vibrail Cloud, or pull a cloud project back to this instance. These are
...
also returns `404` when called on Vibrail Cloud. List your projects with `GET /api/projects` to get valid
IDs.
</Callout>
