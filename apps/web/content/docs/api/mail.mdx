...
The Mail API drives Vibrail's self-hosted email feature: it installs an [iRedMail](https://www.iredmail.org)
...
Every route here is gated by `localOnly` and is **never mounted in Vibrail Cloud** — the module is
...
Most paths carry a `:serverId` (or take one in the body / `?serverId=`). Vibrail checks that the server
...
| `GET /api/mail/servers` | `mail_server:list` | List every server Vibrail has provisioned (or is provisioning) mail on. |
...
<TypeTable
  type={{
    serverId: { type: 'string', description: 'The Vibrail server to install onto.', required: true },
...
If Vibrail lost its record of a mail server (rebuilt desktop, wiped DB), scan the server, then adopt it —
...
- `{ "kind": "self", "serverId": "srv_123" }` — self-hosted on one of your Vibrail servers.
...
Vibrail couldn't reach the server over SSH, or the probe threw. Check the server is online and its SSH
