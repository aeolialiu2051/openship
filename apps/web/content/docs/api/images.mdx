...
<Callout title="Catalog comes from Vibrail Cloud" type="info">
The module is available on every deploy mode (self-hosted, cloud, and desktop), but the catalog itself is
served by Vibrail Cloud. On a **SaaS** instance it uses the master catalog; on a **self-hosted** instance
...
vibrail api /images
vibrail api /images -q search=postgres -q category=database
```
...
[`vibrail token create`](/docs/cli/access) and send it as `Authorization: Bearer <token>`.
</Callout>
