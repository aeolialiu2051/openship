---
title: Cloud API
description: Connect a self-hosted instance to Vibrail Cloud, and the cloud control-plane endpoints (handoff, edge proxy, pages, subgraph transfer, GitHub App proxy) that back it.
---

import { TypeTable } from 'fumadocs-ui/components/type-table';

The Cloud API links a self-hosted Vibrail instance to **Vibrail Cloud** (the managed control plane) so it can deploy to cloud runtimes, use managed domains and pages, and borrow the central GitHub App. It has **two disjoint halves** and which one is mounted depends on the process mode:
...
from [`vibrail token create`](/docs/cli/access)); the dashboard uses your session cookie. The cloud
...
|---|---|---|
| `GET /api/cloud/status` | `cloud:read` | Vibrail Cloud connection status for this instance. |
| `GET /api/cloud/workspaces` | `cloud:read` | List the org's Vibrail Cloud (Oblien) workspaces, with drift (orphaned cloud/local). |
...
PKCE handshake against Vibrail Cloud, then posts the result to `connect-finalize` below. There is no
dedicated `vibrail cloud connect` CLI verb.
...
Present only in cloud mode (`CLOUD_MODE`) — i.e. these run on Vibrail Cloud, not on your self-hosted box.
...
them up by hand, and there is no `vibrail` CLI verb that maps to them one-to-one. The request shapes below
...
The requested slug is owned by another tenant on the shared cloud zone. Oblien returns `409` and Vibrail
