...
description: Manage the storage targets (S3-compatible, SFTP, local, or another Vibrail server) that backups are written to, and run a connectivity preflight.
---

import { TypeTable } from 'fumadocs-ui/components/type-table';

A backup destination is a reusable storage target — an S3-compatible bucket, an SFTP server, a local
filesystem path, or another Vibrail server — that [backup policies](/docs/api/backups) write their archives
...
<TypeTable
  type={{
    ...
    kind: { type: "'s3_compatible' | 'sftp' | 'vibrail_server' | 'local'", description: 'Storage backend. Determines which other fields are required.', required: true },
    ...
    serverId: { type: 'string | null', description: "An existing server's ID to reuse. Required for vibrail_server; must belong to your org." },
...
- **`vibrail_server`** — `serverId` (a server in your org).
...
```bash
# Same thing from the CLI:
vibrail backup destination preflight bkd_123
```
