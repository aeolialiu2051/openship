...
The Permissions API runs Vibrail's team and access-control model: it reports whether the active org is a
...
its `permissions` in place. An **empty `permissions` array revokes** the grant (Vibrail keeps no
