---
title: Terminal API
description: Open an interactive SSH shell to a managed server over a WebSocket, authenticated by a single-use handshake ticket.
---
...
guarded by a local-only middleware. On Vibrail Cloud these paths return **404**. (Cloud service shells go
...
created with [`vibrail token create`](/docs/cli/access)) or the dashboard's session cookie. The WebSocket
...
`vibrail server ssh <serverId>` is stubbed as "coming soon" — the CLI doesn't bundle a WebSocket client, so
...
subprotocols in the `Sec-WebSocket-Protocol` header; the value must be the prefix `vibrail.terminal.v1+`
...
reattach. To resume, present a second subprotocol alongside the ticket: `vibrail.terminal.resume+<resumeToken>`,
