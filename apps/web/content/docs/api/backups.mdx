...
is the project's **Backups** tab, and from the terminal it's the [`vibrail backup`](/docs/cli/projects)
command.
...
and cloud instances; the project-scoped policy and run routes proxy to Vibrail Cloud for cloud projects.
...
vibrail backup policy create --project proj_123 --destination dst_abc \
...
vibrail backup policy run bkp_abc --follow
```
...
vibrail backup run protect run_xyz
vibrail backup run protect run_xyz --release
```
...
vibrail backup run restore run_xyz
vibrail backup restore apply rst_123 --token <confirmationToken>
```

Cancel a staged or in-flight restore with `POST /api/backup-restores/:restoreId/cancel`
(`vibrail backup restore cancel rst_123`).
...
vibrail backup run get run_xyz --follow
vibrail backup restore get rst_123 --follow
```
