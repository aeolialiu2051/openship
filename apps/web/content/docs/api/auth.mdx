---
title: Auth API
description: The Better Auth wrapper plus the desktop session-bootstrap and cloud sign-in handoff, mounted under /api/auth.
---

import { TypeTable } from 'fumadocs-ui/components/type-table';

The Auth API is how Vibrail signs users in and keeps their session. Almost everything here is served by
...
([`vibrail token create`](/docs/cli/access)) and use it as a bearer header. See the
...
(self-hosted onboarding, no password), a miss triggers an inline bootstrap: Vibrail provisions the local admin
...
curl https://your-host/api/auth/get-session \
  -H "Cookie: vibrail.session_token=<session>"
```
