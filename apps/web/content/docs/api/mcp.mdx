---
title: MCP API
description: The Streamable-HTTP JSON-RPC endpoint that exposes Vibrail's API to AI clients as tools, re-authenticating every call.
---

import { TypeTable } from 'fumadocs-ui/components/type-table';

The MCP API is a single **Model Context Protocol** endpoint that lets AI agents (Claude, Cursor, and any
MCP-compatible client) drive Vibrail as a set of tools. Each tool maps onto a permission-tagged REST route,
...
[`vibrail token create`](/docs/cli/access)) **or** an OAuth 2.1 access token that an MCP client obtains for
you. There is no separate MCP credential. See the [API overview](/docs/api) and
[auth model](/docs/security/auth) for details.
</Callout>

Available on both self-hosted and Vibrail Cloud (mounted in the shared route set).
...
| Method | Result |
|---|---|
| `initialize` | Negotiates the protocol version (default `2025-06-18`; also speaks `2025-03-26` and `2024-11-05`) and returns `capabilities: { tools: { listChanged: false } }` and `serverInfo: { name: "vibrail", version: "1.0.0" }`. |
| `ping` | Returns `{}`. |
...
claude mcp add --transport http vibrail https://your-host/api/mcp \
  --header "Authorization: Bearer $VIBRAIL_TOKEN"
```

The tool set is derived from Vibrail's permission-tagged routes that opt in with an `mcp` block; the
...
can start discovery; PAT clients should mint a fresh token with `vibrail token create`.
</Callout>
