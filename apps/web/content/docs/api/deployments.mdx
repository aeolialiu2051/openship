...
this is the deploy wizard and the **Deployments** tab; from the terminal it's [`vibrail deploy`](/docs/cli/projects)
and [`vibrail deployment`](/docs/cli/projects).
...
per-deployment routes (`/:id/...`) transparently proxy to Vibrail Cloud; local deployments are handled
...
vibrail deploy --project proj_123 --branch main
```
...
<TypeTable
  type={{
    ...
    runtimeMode: { type: '"docker"', description: 'Optional explicit marker. Self-hosted workloads always run in Docker; omit it or pass "docker".' },
...
```bash
vibrail deployment get <deploymentId>
```
...
```bash
vibrail deployment redeploy <deploymentId>
```
...
```bash
vibrail deployment rollback <deploymentId>
vibrail deployment pin <deploymentId>
```
...
```bash
vibrail deployment cancel <deploymentId>
```
...
```bash
vibrail deployment keep <deploymentId>
vibrail deployment reject <deploymentId>
```
...
`prepare` with `source: "local"` (or a folder deploy) isn't available on Vibrail Cloud — there's no host
