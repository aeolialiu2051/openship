...
**Services** tab; from the terminal it's [`vibrail service`](/docs/cli/projects).
...
This module is available on both self-hosted and Vibrail Cloud. When `:id` is a cloud project the request
is transparently proxied to Vibrail Cloud; a local project is served by your own instance.
</Callout>
...
vibrail service create -p my-stack db --image postgres:16 --port 5432:5432
```
...
Adds or updates services parsed from a `docker-compose` file. Existing services omitted from the request
are preserved unless the body explicitly includes `replace: true`. Where repository changes diverge from
stored edits, the affected services are marked as **drifted** so you can resolve them explicitly.
...
```bash
vibrail service sync -p my-stack docker-compose.yml
vibrail service sync -p my-stack docker-compose.yml --replace # complete-stack replacement
```
...
```bash
vibrail service restart -p my-stack web
```

Use `GET /api/projects/:id/services/containers` (`vibrail service containers -p my-stack`) to list the
...
```bash
# Same thing from the CLI:
vibrail service env set -p my-stack web DATABASE_URL=postgres://… --env production
```
