...
project, the request is transparently forwarded to Vibrail Cloud.
</Callout>
...
vibrail api /analytics/overview -q projectId=proj_123
```
...
gathered from the edge proxy (Traefik) rather than per-project. Viewing them triggers an on-demand,
...
`/server/:serverId/live` returns `502` when Vibrail can't reach that server's management API (SSH down,
